...
> **2026-09-08 Owner Dashboard "Command Center" revision (Ajit-approved; mockup: `mockups/26-owner-dashboard-command-center.html`):** the Owner Dashboard (`/` for OWNER_ADMIN, `owner-dashboard.tsx`) is recomposed around three stated Owner needs — *understand the business at a glance, see the actions required of him, reach quick actions* — after Ajit rejected the flat tile-stack layout (and two alternative directions; see `.memlog.md` 2026-09-08 entries). Layout, top to bottom: (1) a deep-navy **hero band** carrying the date heading, the existing quick-actions bar unchanged (New Daily Report primary + Employee Payment / Record Advance / Add Purchase / Search `⌘K`), and the seven Today KPIs as compact light-on-dark stats — those with a day-series behind them (sites reporting, labour, expenses) wear 7-day sparklines; (2) **"Needs your attention"** — the existing gap-flags (missing Daily Reports, D7 pending pricing, draft Site Contracts) recomposed as one ranked queue with severity edges and one action per row; every behavioral rule carries over verbatim (each Site individually named and actionable, 3+ missing-report flags fold behind a defaults-open summary, pending pricing is a count never ₹0, count-of-1 deep-links straight to that Purchase's pricing page); (3) a **per-site operations table** — Site, status, today's Daily Report state + submitted time, labour, received, consumed, expenses today, with a reconciling all-sites totals row — replacing the bare site-name preview cards; (4) a **low-stock strip** naming each low material and its remaining quantity inline; (5) the **Money strip** (Expenses This Month, Vendor Outstanding, Outstanding to Subcontractors, Cash Tied Up emphasized as the anchor figure) with the honest-"—" degradation rule unchanged; (6) recently-viewed chips retained. **Scope guard:** this one page only — no other surface, route, component contract, or API behavior changes; the new reads (per-site today breakdown, 7-day trend series) are additive composition endpoints on the existing dashboard module. The zero-Sites whole-page empty state is unchanged. Visual tokens for the band/sparklines: DESIGN.md → Components → Dashboard hero band.
...
| Gap flag list (`GapFlagList`) | Owner Dashboard / Supervisor Home's missing-Daily-Report flags (2026-09-05) | Below 3 flagged Sites, every Gap Flag still renders inline. At 3+, they fold behind one summary line (`DetailsDisclosure`) so a Tenant with many Sites doesn't get a wall of full-width warning rows — but the fold **defaults open**: nothing is hidden on first paint, only grouped under a labeled, user-collapsible header. Defaulting closed was considered and rejected — it would make the more urgent case (more Sites missing) read as *less* visible than two Sites, contradicting "never a silent absence in a list" above. |
...
| Guide step | Help & Guides guide pages | A numbered card per step: step number in a filled `accent-teal-700` circle, one short instruction, a real annotated screenshot with the relevant field/button ringed in `accent-teal-700` (never a full unannotated screenshot, never an icon/illustration standing in for the real screen), a flow-line connector (arrow, `≤160ms` fade-in on scroll into view — same motion ceiling as everything else, no bounce) to the next step. The final step of every guide is a real "Try it yourself" link into the live page it just taught — an actual `<a>`, never a fake button. A guide whose screenshot hasn't been captured yet falls back to text-only rather than blocking the guide from shipping (2026-09-06: this is the rollout state for most guides today — see Guide screenshot pipeline below). |
| Guide screenshot pipeline (2026-09-06) | Every Guide step's screenshot | Captured by an automated script (not a human with an image editor) that drives the real app to the step's screen and captures it directly, drawing the `accent-teal-700` highlight ring by CSS selector — re-run whenever the target screen's layout changes, so a screenshot can never silently drift from the real UI the way a hand-annotated one would. Captures **both** a mobile and a desktop screenshot per step (most guides are read by mobile-first Supervisors, but Owner-facing guides like Reports/Vendors are read on desktop too); the guide page shows whichever matches the reader's own viewport automatically — no manual "view on mobile/desktop" toggle. Rollout priority: the daily-maximum-used flows first (Daily Report submission, Material Consumption/Purchase, Attendance/Work Records), then the master-data CRUD guides (Sites, Subcontractors, Vendors, Payments, Team/Labour) — most of the latter don't exist as guides at all yet and need their step content authored before they can be screenshotted, tracked as separate backlog scope, not blocking this pattern from shipping for the 5 guides that already exist. |
| Guide content source | Help & Guides, Client Presentation | Both surfaces render from one shared content source (module explanations: what/why/who/how/after-save/example; flow stories; FAQ entries) — a guide or example authored once appears in both places automatically. Editing content in two places is a defect, not a maintenance choice. **Field completeness (2026-09-06):** a guide's steps must cover every field on the real form it teaches, one step per field, including optional/secondary ones (Notes, Delivery Location, Vehicle Details, Receiver Name, etc.) — never a curated subset that quietly leaves some fields unexplained. Confirmed a real gap in all 5 shipped guides at the time of this decision (e.g. the 13-field `PurchaseForm` had only ~3 fields covered by its guide) — fixing those 5 to full coverage is in scope alongside the screenshot pipeline above, not deferred with the new master-data guides. A field a guide's flow genuinely never shows (e.g. Rate/Total/Payment Status on a Supervisor's D7 pricing-pending Purchase entry) is correctly absent — this rule is about not skipping fields the reader *will* see, not padding a guide with fields that don't apply to it. |
...
| Mobile card list | Every `DataTable` instance in the product below `md` (as of 2026-09-05, this stopped being an enumerated list — see below) | Each row renders as a card — key facts first, status as text+color pill, the row action always visible. Desktop keeps the full table. No horizontal-scroll-only tables — this is a hard rule, not a per-list judgment call; any list wide enough to clip a column on a 390px screen needs `mobileCard`. A short list of genuinely narrow tables (≤2-3 short columns — e.g. lookup lists, Current Stock, Outstanding Advances) is confirmed exempt; a new `DataTable` defaults to needing `mobileCard` unless it's provably that narrow. |
...
| iOS status bar / notch | `viewport-fit=cover` (set for the standalone/installed PWA) draws content edge-to-edge under the notch and status bar — every fixed top or bottom bar must pad for it with `env(safe-area-inset-top)` / `env(safe-area-inset-bottom)`, not just the bottom quick-bars. This is a hard rule, not a per-bar judgment call: any new fixed-position chrome touching a screen edge needs the matching safe-area inset from the day it ships. |
