...
| Mobile card list | Wide lists (Payments, RMC, Expenses, Movements, Waste) below `md` | Each row renders as a card — key facts first, status as text+color pill, the row action always visible. Desktop keeps the full table. No horizontal-scroll-only tables. |
