...
- `shadow-bar-top` (2026-09-05) — `shadow-2`'s values with the vertical offset negated, for chrome fixed to the *bottom* edge whose content sits above it (the mobile quick-bars) rather than below it. Same elevation weight as `shadow-2`, just pointed the other way.

This is a deliberate escalation from the first Direction C exploration pass, which read as too flat. The rule going forward: **any card or tile that represents drill-down data must carry visible elevation at rest**, not just on hover — elevation is how the product signals "this is a distinct, actionable object," not a decorative afterthought. Dark mode shadows use pure black at higher opacity (`rgba(0,0,0,.4–.5)`) since colored shadows disappear against dark surfaces. The mobile quick-bars were the last flat, hairline-only exception to this rule in the shipped product — closed 2026-09-05.
...
- **Guide step** — a numbered explanation card for Help & Guides: a filled `accent-teal-700` circle holding the step number, a `card-title` instruction line, a real annotated screenshot, and a thin `border-strong` connector line to the next step. `shadow-1` at rest (this is explanatory content, not drill-down data, so it sits one elevation below a primary Card). **Screenshot annotation (2026-09-06):** the relevant field/button is marked with a `2px accent-teal-700` outline ring, offset `4px` outside the element's own edge (never a spotlight/dim-rest treatment, never a drawn arrow) — same hue as every other "this is interactive/important" affordance in the system, so the ring reads as a natural extension of the product's own visual language rather than an added annotation layer. The ring carries a slow (`~2s`) ambient pulse — see the Do's/Don'ts entry below, this is a deliberate, separate motion category from the system's usual ≤160ms transitions, not an exception to that rule.
...
| The one ambient-loop exception (2026-09-06): a slow (`~2s`), low-contrast opacity pulse on a Guide step screenshot's highlight ring — a real "look here" wayfinding aid on unfamiliar content, honoring `prefers-reduced-motion` | Use a looping/ambient animation anywhere else, or make the Guide-step pulse fast, bouncy, or high-contrast enough to feel like a notification badge |
