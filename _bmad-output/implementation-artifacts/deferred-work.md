# Deferred Work

## Deferred from: code review of story-5.1 (2026-08-14)

- `countThisMonth` uses server-local timezone instead of explicit IST — edge-case precision on the "Purchases This Month" stat tile at month boundaries, not central to any AC.
- No cross-field validation that `totalAmount ≈ quantity × rate` on Purchase — would catch typos, but not specified by any AC.
- No `createdByUserId` attribution on Purchase — matches the already-tracked epic-wide "no per-request auth yet" TODO in AGENTS.md; will resolve once `apps/api` validates a Clerk session per-request (AD-10).
- `PurchasesService.list()` has no pagination — systemic pattern across the whole codebase (Sites, Team, Materials lists are all unbounded too), not specific to this story.
- Archived Material/Site referenced by a correction renders the locked dropdown as visually unselected in the correction form — cosmetic only, the hidden input still carries the correct id through to submission.

## Deferred from: code review of story-5.2 (2026-08-14)

- No `sourceSiteId !== destinationSiteId` check for SITE_TO_SITE Movements — a self-transfer would pass validation. Real gap, but SITE_TO_SITE is Story 5.4's own scope; re-flag explicitly under Story 5.4's review.
- `confirmReceipt` semantics for a correction Movement (negative `sentQuantity`) are unaddressed — genuine product-design ambiguity, not specified anywhere in Dev Notes; narrow edge case since Movement corrections are rare.
- No RBAC/role guard on any Movements endpoint — matches the already-tracked epic-wide "no per-request auth yet" TODO in AGENTS.md.
- Movements pages throw a plain `Error()` on fetch failure instead of the shared error state, and have no `loading.tsx` — established pattern across the whole codebase, not specific to this story.
- A Site-to-Site Movement's `correctHref` points at the `/movements/godown-to-site/:id/correct` URL — functionally correct (the page branches on `movement.kind`), but the URL segment is misleading; a proper fix needs a dedicated `/movements/site-to-site/:id/correct` route.
- `getSites`/`getMaterials` fetch helpers duplicated verbatim across several pages — systemic pattern across the whole codebase.
- No upper-bound sanity check on `receivedQuantity` relative to `sentQuantity` in `confirmMovementReceiptSchema` — enhancement, not specified by any AC.
- Quantity `TextField`s have no `min="0"` HTML attribute — cosmetic only, server already validates.
- `movedAt` uses `z.iso.date()` instead of Task 1's literal `z.coerce.date()` — matches Story 5.1's deliberate date-only convention, just undocumented in this story's own Completion Notes.

## Deferred from: code review of story-5.3 (2026-08-14)

- `/movements/vendor-to-site/new/page.test.tsx` doesn't verify `siteId` is posted alongside `destination=SITE` end-to-end — coverage nice-to-have, not a proven defect.
- No empty-state handling on `/movements/vendor-to-site/new` if zero Sites exist yet — narrow UX edge case.
- Quantity `TextField` has no client-side `min="0"` in "new" mode — cosmetic, systemic across Purchase/Movement forms.
- `purchases.service.integration.spec.ts` hardcodes a literal `purchasedAt` date across test cases — minor test-quality nit.

## Deferred from: code review of story-5.4 (2026-08-14)

- No client-side prevention of picking the same Site for both Source and Destination on the transfer form — the server-side guard is now fixed; this is a UX nicety on top.
- Correction routing lives under `/movements/godown-to-site/[id]/correct` for both Movement kinds, and the shared form lives in a kind-named folder — real naming nit, low priority, a proper fix is a larger restructure.
- `getSites`/`getMaterials` fetch helpers duplicated across pages — systemic, already logged under Story 5.2.
- Movements pages throw a raw `Error()` on fetch failure / no shared error state — systemic, already logged under Story 5.2.
- `confirmReceipt` semantics for a correction Movement are unclear — already logged under Story 5.2.
- No client-side guard against a zero-delta correction on the Quantity adjustment field — cosmetic, server already validates.
- `MovementFormInitialValues` has no type-level tie between `kind` and `sourceSiteId` — same class of prop-typing tightening already applied to `PurchaseForm` under Story 5.1, not yet applied here.

## Deferred from: code review of story-5.5 (2026-08-15)

- No pagination on `ConsumptionService.list()` / the combined Movements page — systemic, already logged under Story 5.1.
- No visual lineage for corrections in the combined Movements table (`correctsId`/`reason` not surfaced) — applies uniformly across all four row types, a table-wide enhancement.
- Every row's "Correct" link shares the identical accessible name across the table — real a11y concern (WCAG 2.4.4), pre-existing since Story 5.1's shared `CorrectAction` component.
- `quantity: z.number()` on Consumption has no finiteness/bounds check — enhancement, not specified by any AC.
- No indication when correcting a Consumption that is itself already a correction (correction chains) — enhancement.

## Deferred from: code review of story-5.6 (2026-08-15)

- No filtering/tabs UI on the combined Movements page (`07-movements.html`'s chip row: All/Purchases/Movements/Consumption/Wastage & Returns) — page-wide feature spanning all four transaction types.
- `CorrectAction` renders a plain `<a href>` instead of `next/link`'s `<Link>`, causing full page reloads on every Correct click — pre-existing since Story 5.1's shared component.
- No pagination/date-range scoping on the four merged `findMany()` calls backing the Movements page — systemic, already logged under Story 5.1.
- Corrections aren't visually distinguishable in the combined table (no badge/reference back to the original row) — table-wide gap across all four row types.

## Deferred from: code review of story-5.7 (2026-08-15)

- No pagination on `GET /purchases`/the merged Movements page — systemic, already logged under Story 5.1.
- Fetch failures on the Inventory page bypass AD-6's shared error-state policy — systemic, already logged under Story 5.2.
- `EditMaterialForm`'s `lowStockThreshold` `TextField` has `min={0}` while the server requires strictly positive — minor client/server boundary mismatch, harmless.
- `materials/page.tsx` includes `lowStockThreshold` in its list type but doesn't render it anywhere on the list — enhancement.
- Godown vs Site Stock tables present Material/Size inconsistently (split columns vs one concatenated column) — cosmetic layout inconsistency.
- The GapFlag's "Transfer Stock" action always links to the generic entry route with no pre-selected Material/quantity — real UX gap, but matches Task 3's literal spec.
- `getLowStockMaterials()`'s Decimal→`Number` conversion when summing across Sizes risks float precision loss at extreme magnitudes — theoretical.
- No `aria-label` on stat-tile em-dash placeholders — matches the established pattern on Team/Sites pages.

## Deferred from: code review of story-6.1 (2026-08-15)

- `list()`'s `isToday` check and `getTeamSummary()`'s date boundaries use UTC, not IST — systemic, same pattern already deferred for `PurchasesService.countThisMonth`.
- No server-side check that an assigned `EmploymentType` is `isActive` — currently unreachable since nothing can disable an Employment Type yet (Epic 14 scope).
- `EmploymentType.name` isn't trimmed/case-normalized — matches the same pattern used by `MaterialCategory`/`Unit`/`Site` elsewhere.
- `EditTeamMemberPage` fetches Employment Types even on a 404 Team Member — minor wasted read.
- **Flagged for Epic 7 (not this codebase's own action item, noted for awareness):** `TeamMember.outstandingAdvanceBalance` and `Advance`/`AdvanceAdjustment`/`Payment`'s `correctsId`/`correctionReason`/`reason` columns exist in `schema.prisma` with no corresponding migration — `pnpm db:generate` will produce a client whose fields don't exist in an actually-migrated database. Not exercised by anything in Epic 6; will need its own migration before Epic 7 code goes live.

## Deferred from: code review of story-6.2 (2026-08-15)

- `todayDate()` and the Work Record lock-key timestamps use UTC, not IST — systemic, same pattern already deferred repeatedly.
- `createWorkRecordBatchSchema` doesn't reject a duplicate `teamMemberId` within a batch at the schema layer — functionally caught later by the advisory-lock check, just a less immediate error.
- `GET /work-records`/`default-crew` query params have no format validation — low exploitability, only called by the app's own date picker today.
- The `workrecord:${teamMemberId}:${workDate}` lock-key string is duplicated (with matching comments) across `dsr.service.ts` and `work-records.service.ts` rather than a shared constant.
- No real-Postgres integration test for `WorkRecordsService`'s advisory-lock concurrency — the underlying `lockOnKey` mechanism is already proven via `DsrService`'s own integration test (same shared helper).
- `hours`/`overtimeHours` have no upper bound — enhancement, not specified by any AC.

## Deferred from: code review of story-6.3 (2026-08-15)

**Reviewed while a concurrent session was actively building Story 7.4 on the same files** (`team-members.service.ts`, `team/page.tsx`, `team/[id]/page.tsx`) — findings touching those three files were deferred rather than patched, to avoid colliding with in-flight work. Two agent-raised concerns turned out to already be resolved by that concurrent work (`getTeamSummary()`'s Advance/Payment totals replaced by a materialized-balance-backed `getOutstandingAdvances()`), noted in the story file rather than repeated here.

- Full Advance Ledger UI on the Team Member detail page — real question at review time, but superseded by Story 7.4 landing the real thing on the same page; needs a follow-up look once that work is committed.
- Team summary UI doesn't display weekly/monthly payment totals (AC #3's literal text) — real gap as last observed, but the page is under active edit; re-check once Epic 7's Payments work settles.
- `list()`'s `isToday` check and `getTeamSummary()`'s date boundaries use UTC, not IST — systemic, already logged repeatedly.
- `totalTeamMembers` (active-only) vs. the roster table (`list()`, no active filter) can show different counts with no explanation — cosmetic.
- Neither `WorkRecordsService.create`/`.createBatch` nor `TeamMembersController` checks `TeamMember.isActive` — same class of gap already deferred under Story 6.1.
- `TeamMembersService.list()`/`WorkRecordsService.list()` have no pagination — systemic, already logged repeatedly.

## Deferred from: code review of story-8.1 (2026-08-16)

- No RBAC/role guard on any `apps/assets` endpoint (Machinery/Vehicle/MachineryType/VehicleType) — matches the already-tracked epic-wide "no per-request auth yet" TODO in AGENTS.md.
- `createMachineryTypeSchema`/`createVehicleTypeSchema`/`createMachinerySchema`/`createVehicleSchema` don't `.trim()` string inputs — systemic, matches every existing schema in `packages/shared`, already logged for `EmploymentType.name` under Story 6.1.
- `MachineryType.name`/`VehicleType.name` uniqueness is case-sensitive in Postgres ("Excavator" vs "excavator") — matches the same pre-existing pattern on `MaterialCategory`/`Unit`/`EmploymentType`.
- No index on the new FK columns (`Machinery.typeId`, `Machinery.currentSiteId`, `Vehicle.typeId`, `Vehicle.currentSiteId`) — systemic, the schema has no `@@index` on almost any relation scalar field project-wide, not specific to this story.
- The `20260816070000_add_machinery_vehicle_type_and_current_site_relation` migration adds `typeId TEXT NOT NULL` with no default/backfill, same as the already-shipped `20260813140000_add_employment_type_model` migration for `EmploymentType` — would fail if applied against a table with existing rows. Worth a platform-wide decision on a safe backfill pattern for future free-text-to-lookup-table conversions, not a fix specific to this story.
- Machinery/Vehicle-Vehicles pages throw a plain `Error()` on fetch failure with no shared error state and no `error.tsx`/`loading.tsx` — established pattern across the whole codebase (Team, Materials, Movements), already logged repeatedly.
- Blanking a required Edit field (Name/Type/Asset Number/Number) is silently treated as "leave unchanged" rather than surfacing a validation error, due to the `formData.get(x) || undefined` pattern — same pattern already shipped in `team/[id]/edit/actions.ts`.
- `fetch()` calls in "new"/create Server Actions have no `try/catch` (unlike edit actions, which do) — systemic inconsistency across the whole app between create- and edit-action files, not unique to this story.

## Deferred from: code review of story-9.1 (2026-08-17)

- No pagination or search/filter on `GET /vendors` — systemic, already logged repeatedly (Sites/Materials/Team lists are all unbounded too).
- No authorization guard on `VendorsController`'s mutating endpoints — matches the already-tracked epic-wide "no per-request auth yet" TODO in AGENTS.md.
- Vendor pages `throw new Error(...)` on fetch failure instead of AD-6's shared error-state component — established pattern across the whole codebase, already logged repeatedly.

## Deferred from: code review of story-9.2 (2026-08-17)

- No pagination on `GET /vendors/:id/purchases` — systemic, already logged repeatedly under Story 5.1 and elsewhere.

## Deferred from: code review of story-10.1 (2026-08-26)

- The delta-correction schema pattern (`createRmcEntrySchema`, mirroring `createPurchaseSchema` from Epic 5) lets the quantity go negative on a correction but keeps `ratePerM3`/`totalAmount` (`rate`/`totalAmount` on Purchase) `.positive()` unconditionally — a downward correction can't express a negative cost delta, so it silently inflates `RmcService.statsThisMonth`'s (and Purchase's equivalent) monthly totals instead of reducing them. Pre-existing since Epic 5's Purchase; RMC just faithfully inherited the same gap.
- No list page anywhere in the app (Purchase, Movement, Consumption, Payment, Asset Movement, and now RMC) shows any indicator — badge, icon, or `reason` tooltip — that a row is itself a correction rather than an original entry. Systemic, cross-epic UI gap, not introduced by this story.
- `GET /rmc-entries?date=`/`?siteId=`/`?vendorId=` (and `DsrService.listByDate`'s identical pre-existing `date` param) do no format validation — an unparseable date string reaches `new Date()` unguarded, and a duplicate query key parses to an array Prisma wasn't expecting — both would surface as a raw 500 instead of a 400. Matches the codebase's existing, already-unvalidated `@Query()` string param convention.

## Deferred from: code review of story-10.2 (2026-08-26)

- `GET /rmc-entries/report`'s new `from`/`to` (and `groupBy=` empty-string) params are unvalidated — a malformed `from`/`to` reaches `new Date()` and flows into the Prisma `deliveredAt` filter as an Invalid Date. Same systemic unvalidated-`@Query` class already logged under 10.1; additionally the `from`/`to` window is spec'd at the API but not yet wired to any UI control, so it's currently unreachable from the app.
- `RmcService.report`'s daily slice buckets by UTC calendar day (`toISOString().slice(0,10)`), not IST — matches `RmcService.list`'s existing single-day boundary and the systemic UTC-not-IST issue logged repeatedly across Epics 3/6/7.
- `getRmcReport` (like the sibling `getRmcEntries`/`getRmcStats` on the same page, and every list page in the app) `throw`s on a non-OK fetch instead of rendering AD-6's shared error state — established whole-codebase pattern, already logged repeatedly. The story's specific AD-6 obligation (a genuine empty state for an empty range/tenant) IS implemented.
- `RmcService.report` loads all matching `RmcEntry` rows and aggregates in application code rather than pushing a `GROUP BY`/`SUM` to Postgres — a deliberate choice (spec: live aggregate, no materialized rollup; makes exact reconciliation unit-testable and handles day-truncation Prisma `groupBy` can't) but unbounded over a very large history. Same systemic no-pagination/scale concern already logged repeatedly.

## Deferred from: code review of story-11.1 (2026-08-26)

- **FR-41 "optional document/attachment" is not implemented** — no shared file-upload primitive exists in `packages/ui` (only the bespoke DSR photo/R2 presign flow), so the Expense form scopes it out per the story's own Task 4 allowance. Genuine AC-adjacent follow-up: add a reusable upload primitive (reusing the Epic 3 DSR photo path) and an `Expense` document field. Not silently dropped — tracked here.
- `GET /expenses`' `siteId`/`categoryId`/`from`/`to` filters are fully built at the API but the web list page fetches with no query string and renders no filter controls — same "queryability at API only, no UI" pattern already logged for RMC 10.2. Story Task 4 didn't require filter UI.
- Expense corrections (like every other correction form in the app — Purchase, RMC, Advance, Payment) never surface `reason`/`correctsId` in any list UI, and a correction's `incurredAt`/date is editable (can book a delta into a different month than the original, distorting `summary()` buckets). Systemic across all correction ledgers, not unique to Expenses.
- `ExpensesService.summary()`/`list()` month/week boundaries mix server-local and UTC (`new Date("YYYY-MM-DD")`) date math — same systemic UTC-not-IST issue logged repeatedly; on the IST deployment, edge-of-period expenses can bucket into the wrong week/month.
- `Expense` records no `createdByUserId`/`correctedByUserId` attribution — matches the epic-wide "no request-level auth in apps/api yet" TODO in AGENTS.md; weakens the AD-9 audit trail (proves what/why, not who).
- `ExpensesService.list()` is unbounded (no pagination) — systemic, already logged repeatedly.
- The seed replaces 4 old placeholder categories with the 9 named defaults but never removes the old rows (upsert-by-name), so a previously-seeded environment ends up with 13 categories. Low-impact (fresh-provision seed), no reconciliation path.
- `translateWriteError`'s P2003→400 branch (bad `siteId`/`categoryId` on a fresh create) and `summary()`'s month/week boundary `where`-clause math are both unasserted by the current mocked-Prisma specs — the RMC equivalents did cover these; test-strengthening follow-up.

## Deferred from: code review of story-12.1 (2026-08-26)

- The web dashboard heading hardcodes `timeZone: "Asia/Kolkata"` while the API derives "today" from the configurable `APP_TIMEZONE` — they agree at the default but would disagree if `APP_TIMEZONE` is changed. Matches the whole web app's pervasive hardcoded `en-IN`/`₹`/IST formatting (single-market India launch); make the web share the configured zone when the product goes multi-timezone.
- `local-day.ts`'s `zonedMidnightUtc` samples the zone offset at the UTC-midnight guess rather than at the resulting local-midnight instant — off by an hour near a DST transition. Not exercised: the default/only zone (`Asia/Kolkata`) is DST-free; revisit if a DST deployment zone is ever configured.
- No authorization guard on `GET /dashboard/today` (FR-35 scopes it to Owner/Admin) — matches the epic-wide "no request-level auth in apps/api yet" TODO; Epic 14 Story 14.2 wires the real `ClerkAuthGuard`.
- The dashboard page throws on a failed fetch (via `getJSON`) with no AD-6 error/loading state and `Promise.all` is fail-fast (one aggregate failing collapses the page) — established whole-codebase pattern, already logged repeatedly; the empty/zero states this story needs ARE handled.
- Test-strengthening: no case asserts `getToday()` with no args falls back to `resolveAppTimeZone()`/current time, no zero-active-Sites case, and only Purchase's day-range `where` is asserted (consumption/rmcEntry/expense range filters aren't) — core behavior is covered, these harden it.
- `sitesReportingToday` counts DSRs from Sites of any status while `sitesMissingDsrToday` is scoped to ACTIVE Sites — computed against different populations. Spec-conformant (Task 1 defines each that way), but a COMPLETED Site that reported would inflate the count; worth a product decision if it ever matters.

## Deferred from: code review of story-12.2 (2026-08-26)

- `outstandingAdvances.teamMemberCount = byTeamMember.length` counts every Team Member the outstanding-advances endpoint returns (including ₹0 balances), so it can exceed a "N with an open balance" reading. Left exactly as Task 1's explicit transform dictates; flip to a positive-balance filter if the other semantics are wanted.
- The zero-Sites whole-page empty state gates on total Sites (`sitesPreview.length === 0`) rather than the spec's literal `activeSites.count === 0` — a deliberate, better reading (a Tenant with only Completed/On-Hold Sites shows "0 active", not the create-first-Site screen; the two coincide for the brand-new-Tenant case). Documented in the story; noted here in case the literal reading is later preferred.

## Deferred from: code review of story-13.1 (2026-08-26)

- **Cron "today" is computed in UTC and the schedule hour (`0 18 * * *`) is a placeholder** — genuinely tangled with an unmade PO decision (PRD Open Question: delivery time + timezone). Should reuse Story 12.1's `local-day.ts` helper for the compile day boundary once resolved, so reports compile on the deployment-local calendar day like the Dashboard. Flagged in `vercel.json` `$comment` and the controller.
- **WhatsApp channel** is blocked on the deferred BSP/vendor contract (PRD Open Question 3) — implemented behind a `NotConfiguredWhatsAppSender` that fails honestly (in-app-visible reason) and is left out of the default enabled set; no vendor wired.
- Read endpoints `GET /reports/daily[/:id]` have no auth guard and the report `content` carries site financials/labour/expenses — matches the epic-wide "no request auth in apps/api yet" TODO; Epic 14 Story 14.2 wires `ClerkAuthGuard`. The report preview/log pages fetch without credentials.
- No validation on `?date=`/`?from=`/`?to=` on the reports endpoints — same systemic unvalidated-`@Query` class already logged; a malformed value reaches `new Date()`/Prisma unguarded.
- No indexes on `ReportDelivery.status`/`attempts` (retry sweep), `DailyReport.generatedAt` (log ordering), or `reportDate` (filters); `retryPending` has no `take`/ordering — systemic no-index/no-pagination pattern already logged repeatedly.
- The compiled `content` snapshots `work.inProgress`/`planned`/`issuesBlockers`/`safetyObservations`/`notes`, but neither the email nor the `[id]` preview renders beyond `work.completed` — safety observations and issues/blockers (arguably the most important DSR lines) are stored but not surfaced. Design-driven follow-up (confirm against the `16-reports.html` report-preview field set) rather than a patch here.
- The report detail page footer renders a FAILED delivery in the same muted `text-ink-500` as others (no danger color/icon) — AC #3's visible-failure requirement is met by the Recent-Reports list badge, but the detail view undersells a hard failure. Minor UI polish.
- `ReportContent` DTO is hand-redeclared in `apps/web` (and `formatDate`/`formatDateTime` duplicated across the two report pages) — established cross-boundary duplication pattern, already the norm across the app.
- `DailyReport` uses `generatedAt` while `ReportDelivery` uses `createdAt` for the same concept, and `ReportDelivery` has no `updatedAt` despite mutating on retry — minor schema-naming/consistency nit.

## Deferred from: code review of story-13.4 (2026-08-26)

- The Financial report sums `material` (SUM Purchase.totalAmount) and `expenses` (SUM Expense.amount) as independent categories per the spec. If a future integration ever populates `Expense.purchaseId` (an Expense that IS a Purchase's own cost entry — reserved but never set today, since Story 11.1 explicitly left it null with no code path to populate it), that cost would be counted in BOTH categories, double-counting it in `total`. Not a bug today (no data can have `purchaseId` set); when that integration is built, the financial report must decide whether purchase-linked Expenses are excluded from the `expenses` category to avoid the double-count. Flagged in the story's Completion Notes.
- `GET /reports/financial`'s `from`/`to` params are unvalidated (same systemic unvalidated-`@Query` class already logged) — a malformed date reaches `new Date()`/`dateRangeBounds` unguarded.

## Deferred from: code review of story-1.8 (2026-08-26) — auth security hardening

These are real security-hardening items that need config/ops decisions (env, provisioning) that don't exist yet, or are spec-permitted trade-offs; none affect correctness for the invite-only single-tenant model this story ships. Revisit when real Clerk keys + the tenant's web origin are wired.

- **`verifyToken` is called without `authorizedParties` (azp).** Clerk recommends passing the allowed frontend origin(s) so a token minted for a different origin/app can't be replayed against this API. Needs the deployment's web origin in env (e.g. `CLERK_AUTHORIZED_PARTY`), which provisioning doesn't set yet. Add `authorizedParties: [...]` to the `verifyToken` call once that env exists.
- **Session revocation isn't honored until token expiry.** Networkless `verifyToken` accepts a signed, unexpired token even after the session/user was revoked server-side. Spec-permitted (`verifyToken` OR `authenticateRequest` — implementer's choice); Clerk session tokens are short-lived so the window is small. If tighter revocation is needed, switch to `authenticateRequest`/handshake.
- **No fail-fast when `CLERK_SECRET_KEY` is unset.** A misconfigured deployment silently 401s every request. Add a startup assertion (env validation) so misconfig is loud, not a confusing all-401.
- **Web helper base-URL `?? ""` fallback.** If `API_URL`/`NEXT_PUBLIC_API_URL` is unset, the client helper would fetch a relative path against the web app's own origin (attaching the bearer token to the wrong host). Config-dependent (build passes with the env set); add a guard that the API base URL is configured.
- **First-user-→-OWNER_ADMIN provisioning is trust-on-first-use with a (very low-likelihood) concurrent race.** Two brand-new users hitting a fresh DB in the same instant could both read `count()===0` and both become `OWNER_ADMIN` (the P2002 recovery only catches same-clerkId collisions). Spec-conformant (the story prescribes first-user→OWNER_ADMIN and an invite-only Clerk instance provisioning ~2 real users). Precondition to document: the Clerk instance must be invite-only; Epic 14 Story 14.2's webhook + admin UI becomes the real role authority. Tighten with a transaction/advisory-lock if the user model ever opens up.
- `@CurrentUser()` casts `req.user as AuthUser`; on a `@Public()` route (guard skipped) it would be `undefined`. Only used on protected routes today; a defensive throw would fail louder if ever misapplied.
- source_spec: `_bmad-output/implementation-artifacts/1-9-installable-pwa.md`
  summary: Offline in-app (client-side) navigations have no offline fallback — only full-page loads fall back to /offline.
  evidence: The SW serves /offline only for `mode === "navigate"` (hard loads/reloads). App Router client navigations fetch RSC payloads (`GET /route?_rsc=…`) which are correctly NetworkOnly (Fix A) and, when offline, reject with no graceful fallback (returning /offline HTML to an RSC fetch would break Next's router). A proper fix is an app-level offline-aware error boundary, out of scope for the SW-only story. Spec-conformant: the frozen I/O matrix only promised navigate-mode fallback.
- source_spec: `_bmad-output/implementation-artifacts/1-9-installable-pwa.md`
  summary: The service-worker static-asset cache (`azentis-shell-v1`) grows unbounded across deploys — old hashed /_next/static chunks are never purged.
  evidence: `CACHE_NAME` is a fixed literal and `activate` only deletes caches with a different name, so cache-first writes from prior builds accumulate in the same store forever (no per-build version, size cap, or eviction). Low severity (browsers evict under storage pressure); a real fix needs a build-time cache-busting version injected into sw.js, which the hand-authored/no-bundler setup doesn't currently provide.

## Deferred from: code review of story-14.2 (2026-08-27)

- **Broad API-wide authorization across Epics 2–13 remains out of scope** (as the story's own Dev Notes state). Story 1.8's global ClerkAuthGuard now authenticates every route, but only the Users-admin endpoints carry an OWNER_ADMIN `@Roles()` check; the other epics' write endpoints are authenticated but not role-gated. A dedicated pass to apply `@Roles()` where FR/role rules require it is real, named follow-up — not silently folded into this story.
- **`svix` v2 is ESM-only while `apps/api` builds to CJS.** Node 22's `require(ESM)` covers it and nothing runnable regresses (the dev-server boot is separately broken per AGENTS.md), but the live Clerk→Svix webhook round-trip is unverified — no real Clerk/webhook secret exists in any environment yet. Verify the actual signed round-trip once a real Clerk instance + `CLERK_WEBHOOK_SECRET` exist.
- **`user.deleted` is a deliberate no-op** — deleting the local `User` row would break every `submittedByUserId`/`recordedByUserId` FK to the departed user's historical records. Full deprovisioning (preserve vs reassign historical authorship, disable access) is a real product decision no AC asks for; flagged as follow-up.

## Deferred from: code review of stories 14.3–14.5 (2026-08-27)

- **14.5 scheduled-report delivery is a branded notification envelope**, not a fully-rendered per-type email body — the Epic 13 report endpoint fetch runs and gates `lastRunAt` on success, but the delivered message carries the report type + date window in a header rather than a rich per-type template. Follow-up (richer per-type templates), consistent with 13.1's "senders never exercised against a real provider" posture. No real Resend/WhatsApp round-trip has run in any environment.
- `run-report-schedules` and the other reports cron routes share the same unvalidated `?date`/frequency-window date handling and UTC "today" as 13.1 — the same PO timezone decision already logged for Epic 13.

## Deferred from: code review of epic-15 stories 15.3-15.5 (2026-08-30)

- Transaction writes (DSR/consumption/expense/RMC/waste-disposal) accept a soft-deleted siteId/vendorId — UI-unreachable today (pickers filter deleted rows); revisit alongside the role-lockdown work.
- Invalid from/to query strings reach Prisma as Invalid Date and 500 instead of 400 — pre-existing dateRangeBounds pattern shared by every report endpoint; fix centrally in dateRangeBounds when touched next.
- User FK from AuditLog RESTRICTs any future hard user delete — no user-delete endpoint exists; if one is added, map P2003 to a 409 or make removal deactivate-only.
- toLocaleString without an explicit timeZone across older pages renders server-zone dates on Vercel (UTC); audit page fixed now, sweep the rest when touching those pages.
- Restore/undelete path for soft-deleted Sites/Vendors — decision 2026-08-30: accepted one-way for now (Owner-only + double-confirmed; SQL recovery). Add a 'Deleted items' restore view if mis-deletes actually occur.

## Deferred from: code review of story-16.1 (2026-08-31)

- No loading state shown during search/sort/page transitions on any of Story 16.1's converted list pages (AD-6's full loading/empty/success/error/validation-failure set) — pre-existing, app-wide: no `loading.tsx` exists anywhere in this app, so every route relies on Next's default RSC transition behavior. Not unique to this story; revisit if/when the app adds route-level loading states generally.

## Deferred from: code review of story-16.2 (2026-08-31)

- `MaterialsService.searchCandidates` filters `Material.isActive` but not the parent `MaterialCategory.isActive` — a Material whose Category has since been disabled can still surface in global search. Consistent with Story 14.3's existing precedent (disabling a Category doesn't cascade to hide its Materials elsewhere either); revisit only if that broader precedent ever changes.
- Global search's Materials matching is name-only, never Category name — a query for "Binders" returns nothing even though the result shows a Binders category. Not an AC violation (AC #1 scopes Material matching to name only) — a reasonable future enhancement.

## Deferred from: code review of story-16.3 (2026-08-31)

- `GET /stock/material/:materialId` returns `200 []` for both a nonexistent Material and a real Material with zero stock — indistinguishable at the API level. The web availability page already handles the 404 case separately, so no current consumer is affected; revisit only if a future consumer calls this endpoint directly.

## Deferred from: code review of f9c01a3..4ebea08 (2026-09-02)

- DW-CR-1: POST /purchases still accepts rate/totalAmount/paymentStatus from a SITE_SUPERVISOR — D7's pricing split is UI-enforced on the create path (PATCH is role-guarded). Server-side hardening: reject pricing fields for supervisor role in the controller.
- DW-CR-2: Money-correction sum semantics are ambiguous for Purchase: correction rows carry a positive restated totalAmount while vendor aggregates SUM totalAmount across all rows (double-count). Pre-existing; RMC was fixed to signed deltas — Purchase needs the same decision.
- DW-CR-3: Pricing an original Purchase that already has quantity-correction rows prices the gate-recorded quantity, not the net corrected quantity — the pricing page shows the original qty read-only. Semantic edge; decide whether Total should derive from net quantity.
- DW-CR-4: currentRole() folds any /users/me failure (incl. expired-session 401) into SITE_SUPERVISOR instead of redirecting to sign-in — an expired Owner silently sees the Supervisor UI. Pre-existing pattern from layout.tsx.
- DW-CR-5: OwnerDashboard's vendor-outstanding does one unbounded parallel purchase-summary fetch per Vendor — fine at current scale, needs batching/an aggregate endpoint as vendors grow. Pre-existing.
- DW-CR-6: useClientValidation only revalidates on submit — a fixed field keeps its stale inline error until resubmit; add per-field revalidation on change/blur. Also: shared schema correction messages still say "quantity delta must not be zero" — delta vocabulary the D4 UI deliberately hides.
- DW-CR-7: DsrDesktopForm's mode="new" branch is dead (its only route was deleted) — prune the mode or restore a desktop-entry caller; dead branch still carries pre-rename strings until pruned (strings fixed in review patch, pruning deferred).
- DW-CR-8: Test-quality follow-ups: assert HELP_CONTENT contextualHelp keys exist (typo = silently missing bubble); several page tests weakened to getAllByText(...).length > 0 (can't catch triplicate rendering); two advance/adjustment test names still describe the old delta UI.
- DW-CR-9: Consider explicit pricedAt/pricedByUserId columns on Purchase for first-class pricing attribution (currently derivable only from the audit log's PATCH entry).

## Deferred from: e2e test suite build-out (2026-09-02)

- DW-E2E-1: Browser console warning surfaced during real e2e runs of the Purchase-recording flow: "flushSync was called from inside a lifecycle method. React cannot flush when React is already rendering." Fires on the Movements list after a real Purchase submission (`apps/web/app/(app)/movements/movements-list-client.tsx` or a state update it triggers, possibly via the router-refresh/flash-toast path). Did not affect functional correctness — the row still rendered correctly with the right pending badge in every observed run — but it's a real React anti-pattern warning that unit/component tests (mocked routers/fetches) would never surface, only a real browser against a real server catches it. Worth tracking down when next touching that file.

## Deferred from: code review of story-19.1 (2026-09-02)

- source_spec: `_bmad-output/implementation-artifacts/spec-19-1-advance-quick-entry-modal.md`
  summary: Dialogs across the app (the new AdvanceQuickEntryModal, plus the pre-existing ConfirmDialog used by every money form) can be dismissed via Cancel/backdrop/Escape while a submission is still pending, with no guard — a race between the in-flight request resolving and the dialog unmounting.
  evidence: Blind Hunter and Edge Case Hunter both independently flagged this on story-19.1's new modal; confirmed the identical unguarded pattern already exists in `packages/ui/src/components/confirm-dialog.tsx` and every form that uses it (AdvanceForm, PaymentForm) — not a regression unique to this story, a systemic gap worth fixing once for every dialog rather than per-story.
- source_spec: `_bmad-output/implementation-artifacts/spec-19-1-advance-quick-entry-modal.md`
  summary: `todayDate()`'s `new Date().toISOString().slice(0, 10)` reads the UTC calendar date, which shows the previous day's date for IST users in the ~5.5-hour window after UTC midnight (00:00-05:30 IST).
  evidence: Blind Hunter caught this on the new AdvanceQuickEntryModal; verified the identical implementation already exists verbatim in the pre-existing `apps/web/app/(app)/team/[id]/advances/advance-form.tsx` — a pre-existing bug this story's modal copied, not introduced. Worth a shared, IST-correct date helper.
- source_spec: `_bmad-output/implementation-artifacts/spec-19-1-advance-quick-entry-modal.md`
  summary: `ComboboxField` pickers (Team Member, Site, etc.) show the generic "No matching X" message even when the underlying list is genuinely empty (zero records exist yet), rather than a distinct "you have no X yet — add one first" empty state per AD-6.
  evidence: Blind Hunter caught this on the new modal's Team Member picker; verified the identical `emptyMessage="No matching Team Member"` already ships unchanged in the pre-existing `apps/web/app/(app)/payments/payment-form.tsx` — pre-existing across every Team Member combobox, not unique to this story.

## Deferred from: code review of epic-19 stories 19.2-19.8 (2026-09-02)

- source_spec: `_bmad-output/implementation-artifacts/spec-19-2-global-search-and-action-palette.md`
  summary: `PaymentSearchResult.netPayable` and `ExpenseSearchResult.amount` are fetched and piped through `search.service.ts` and the shared types, but `global-search.tsx`'s row mapping for those two groups never renders either field — the amount that would help disambiguate two similarly-named entries is silently dropped in the UI.
  evidence: Confirmed via `apps/api/src/search/search.service.ts`'s Payment/Expense mapping (both compute the numeric field) vs. `global-search.tsx`'s `groups` array (neither group's `description` includes it). Cosmetic/incomplete, not a correctness bug.
- source_spec: `_bmad-output/implementation-artifacts/spec-19-5-pending-pricing-deep-link.md`
  summary: TOCTOU gap between `owner-dashboard.tsx` reading `pendingPricingCount` and the follow-up `GET /purchases?pendingPricing=true` fetch when the count is exactly 1 — `purchases[0]` is trusted without re-verifying the set is still exactly one row at the moment of the second read.
  evidence: Edge Case Hunter flagged this; the window between the two server-side reads within one request is milliseconds, and the fallback (empty/failed fetch → generic filtered list) already degrades safely, so the practical exposure is a rare stale link, not data corruption.
- source_spec: `_bmad-output/implementation-artifacts/spec-19-6-recently-viewed-shortcuts.md`
  summary: `recently-viewed-chips.tsx` renders chips without checking the target record still exists — a chip for a since-soft-deleted Site/Vendor/Team Member/Subcontractor links to a 404.
  evidence: Edge Case Hunter confirmed via `recently-viewed-chips.tsx`'s render path (no existence check) and the app's established soft-delete pattern (deleted rows are hidden from pickers/lists but not purged). A full fix needs either a batch existence-check fetch on every Dashboard render (extra API cost) or a soft-delete-aware search endpoint; deferred as a real but low-severity UX edge case (a "recent items" list showing a stale entry is a common, accepted pattern elsewhere).
- source_spec: `_bmad-output/implementation-artifacts/spec-19-2-global-search-and-action-palette.md`
  summary: Documentation drift — `EXPERIENCE.md`/`DESIGN.md` (cited as authoritative references by every Epic 19 story spec) were not updated to reflect the as-shipped search coverage, curated Actions list, or the `action-button-row`/mobile-card implementation details.
  evidence: Blind Hunter noted this; no functional impact, but future stories citing these docs as ground truth should have their assumptions re-verified against code rather than the docs.

## Deferred from: code review of story-16.5/story-16.6 (2026-09-03)

- source_spec: `_bmad-output/planning-artifacts/stories/phase-8-post-launch/epic-16-search-and-scale/story-16.6-search-full-entity-coverage.md`
  summary: 14 new entities' searched columns have no `pg_trgm` index yet, in some tension with the epic's own stated constraint ("do not add or change indexes... unless a genuinely new, currently-uncovered column is introduced").
  evidence: Acceptance Auditor + Blind Hunter both flagged this. Already disclosed as a known follow-up in the story's own Completion Notes — a deliberate, disclosed tradeoff, not silently hidden. The stated justification ("same reasoning as 16.5's indexing note") is imprecise: 16.5's case already had an index via a different mechanism (`pg_trgm`, commit `14bc517`); here there is genuinely no index at all yet. Escalate only if real query volume on these new columns shows it's needed.
- source_spec: `_bmad-output/planning-artifacts/stories/phase-8-post-launch/epic-16-search-and-scale/story-16.6-search-full-entity-coverage.md`
  summary: The new `apps/api/src/search/search-candidates.integration.spec.ts` is gated by `describeIfDb`/`DATABASE_URL` and never runs in the actual CI pipeline (no Postgres service, no `DATABASE_URL` in `.github/workflows/ci.yml`).
  evidence: Verification Gap flagged this. Same pre-existing limitation every other `*.integration.spec.ts` in this repo already has (e.g. `purchases.service.integration.spec.ts`), not introduced or worsened by this diff — the story's "verified" claim is accurate only for local runs with `DATABASE_URL` set.

## Deferred from: multi-goal split of spec-inline-master-data-quick-create (2026-09-03)

- source_spec: `_bmad-output/implementation-artifacts/spec-inline-master-data-quick-create.md`
  summary: Inline "+ Add" quick-create for Material Category, Unit, and Employment Type — plus wiring those as nested quick-create options inside the Material and Team Member quick-create modals — is deferred to a follow-up spec.
  evidence: Bundling all seven entities (Vendor, Subcontractor, Material, Team Member, and their own FK dependencies) pushed the spec to ~2,900 tokens, well past the 1600-token risk threshold. The four entities the user explicitly named (Vendor, Subcontractor, Material, Team Member) form one coherent, independently shippable increment around the new `ComboboxField`/`QuickCreateModal` primitives; the nested nested-FK case (Category/Unit/Employment Type, each needing their own quick-create modal wired one level deep into the Material/Team-Member modals) is a distinct, separable increment. Until this follow-up ships, `materials/new/new-material-form.tsx` and `team/new/new-team-member-form.tsx` keep today's existing "no Categories/Units/Employment Types yet — create one first" blocking behavior unchanged, and the Material/Team-Member quick-create modals built in the main spec show plain (non-creatable) Category/Unit/Employment-Type comboboxes — no regression from today, just not yet improved for this specific nested case.

## Deferred from: implementation of spec-inline-master-data-quick-create (2026-09-03)

- source_spec: `_bmad-output/implementation-artifacts/spec-inline-master-data-quick-create.md`
  summary: The Material quick-create modal collects an extra required "Size / Specification" field and `createMaterialQuickAction` does a two-step `POST /materials` then `POST /materials/:id/sizes`, returning the new MaterialSize's id — not the bare Material's id — as `{ id, name }`. Not explicitly specified anywhere in the spec.
  evidence: Every wired Material picker (Purchase/Consumption/Movement/Return-Wastage/DSR) actually selects a `materialSizeId`, not a Material id (confirmed via `use-dsr-reference-data.ts` and each form's `MaterialSizeOption[]` prop) — a Material has zero Sizes until one is separately added via the edit-page's `SizesSection` (FR-5, purely additive). A quick-created Material with no Size would satisfy "record created" but violate the frozen edge-case matrix's "prepended + selected in parent picker" requirement (it wouldn't even appear as an option). Chose to fold Size collection into the modal rather than leave the new Material unusable; flagging since it wasn't called out in the original spec's Code Map ("apps/api/.../*.service.ts ... no API changes needed" undercounted this).
- source_spec: `_bmad-output/implementation-artifacts/spec-inline-master-data-quick-create.md`
  summary: `QuickCreateModal`'s inner `<form>` needed an explicit `event.stopPropagation()` on submit — without it, submitting the quick-create modal also bubbled a "submit" through React's tree to the originating picker's own parent `<form>` (React bubbles through the component tree across a Dialog's portal, not the DOM tree), running that outer form's own client validation against the wrong FormData. Fixed in `packages/ui/src/components/quick-create-modal.tsx`; `AdvanceQuickEntryModal` never hit this because it's never nested inside another `<form>`. Worth a shared lint/test guard if more portal-based forms get nested inside other forms later.
- source_spec: `_bmad-output/implementation-artifacts/spec-inline-master-data-quick-create.md`
  summary: Every quick-create action (`createVendorQuickAction`/`createSubcontractorQuickAction`/`createMaterialQuickAction`/`createTeamMemberQuickAction`) is a plain Next.js Server Action requiring live connectivity — same as every other standalone entry form's Server Action (per AGENTS.md's existing TODO, the offline queue only covers the DSR submission itself, not standalone forms). A Supervisor mid-DSR with no signal who taps "+ Add Vendor" gets a network-error `formError`, not a queued/retried quick-create. Not a regression — matches today's behavior for every other write in these forms — but worth folding into the existing offline-queue-extension TODO instead of tracking separately.

## Deferred from: spec-preserve-form-data-on-error review (2026-09-03)

- source_spec: `_bmad-output/implementation-artifacts/spec-preserve-form-data-on-error.md`
  summary: `apps/web/app/(app)/_components/category-row-actions.tsx` has no test file at all, despite being the single shared rename/disable-enable control behind four category families (Employment/Machinery/Vehicle Types, Expense Categories) and now carrying two `usePreventFormResetOnError` wirings (rename form + toggle form).
  evidence: Confirmed via `find` — no `category-row-actions.test.tsx` exists anywhere in the repo. Pre-existing gap (the component had zero test coverage before this diff too), surfaced incidentally by this review's blind-hunter pass. Not fixed here to avoid scope creep beyond the form-reset fix; the highest-leverage fix would be one test file covering both forms' happy path and error-preserves-values path.

## Deferred from: intent split of "Site photo upload + lightbox preview" (2026-09-03)

- source_spec: none
  summary: Direct-to-Site photo upload (independent of any Daily Site Report) is deferred to its own follow-up spec — this session only ships the click-to-preview lightbox.
  evidence: Investigation found `Photo.dailySiteReportId` is a required (non-nullable) FK at every layer — Prisma schema, both `presign`/`confirm` Zod schemas, the storage service, and the `getSitePhotoGallery` join query that backs both `GET /sites/:id/photos` and the Reports page. Supporting a DSR-less photo needs a real schema migration (nullable `dailySiteReportId` + new optional `siteId`) plus updating that shared join consistently across both call sites, and at least one small product decision (what date/caption a DSR-less photo displays, since today's gallery sorts/labels by `dailySiteReport.reportDate`). The lightbox, by contrast, is a frontend-only change to one shared component (`PhotoGalleryGrid`) with three callers that all already pass the identical data shape — genuinely independent, much lower risk, ships today.

## Deferred from: code review of story-20.1 (2026-09-04)

- source_spec: `_bmad-output/implementation-artifacts/spec-20-1-twa-wrapper-build-template-and-signing.md`
  summary: Ship (commit is done, deploy is not) the `apps/web/proxy.ts` `/icons` allowlist fix — production TWA/PWA installability against a real tenant manifest is blocked until it deploys.
  evidence: Discovered running a real `bubblewrap init`/`build` against the live `sandeepent.azentis.in` manifest for this story — icon fetches 307-redirected to `/sign-in` HTML because `/icons/*` (no dot in the path, so not matcher-excluded like `/manifest.webmanifest`) hit the session-cookie guard. Same failure mode blocks real Chrome/Android installability checks, not just this build tool. Fix committed separately at `f759313` per explicit user decision to keep it out of epic-20's diff; not deployed.
- source_spec: `_bmad-output/implementation-artifacts/spec-20-1-twa-wrapper-build-template-and-signing.md`
  summary: `infra/android/build.ts`'s manifest fetch (inside `TwaManifest.fromWebManifest`) has no timeout, and `--manifest-url` isn't restricted to expected tenant domains — both fine for today's manually-run, human-supervised CLI, but worth revisiting once Story 20.4 wires this into an unattended provisioning pipeline where a human can't Ctrl+C a hang and the URL comes from config rather than a typed, reviewed argument.
  evidence: Edge Case Hunter and Blind Hunter both flagged these independently; confirmed by reading `build.ts` — no `AbortSignal`/timeout wraps the fetch, and URL validation only checks well-formed `https://`.
- source_spec: `_bmad-output/implementation-artifacts/spec-20-1-twa-wrapper-build-template-and-signing.md`
  summary: No unit tests for `infra/android/build.ts`'s pure validation logic (`parseArgs`, `resolveKeystore` precedence/validation) — matches the existing no-tests precedent for `infra/provisioning/provision.ts`, but this script now carries meaningfully more branching logic than that skeleton.
  evidence: Verification Gap reviewer confirmed via repo-wide search: no test file exists for `infra/android/`, and `infra/` isn't a pnpm workspace member so it's outside `turbo run test`/CI entirely (same as `infra/provisioning/`) — a missing-test-file alone isn't a reportable gap per that review's own rules (no regression-provable consumer), but worth a deliberate follow-up given the script's growing complexity.
- source_spec: `_bmad-output/implementation-artifacts/spec-20-1-twa-wrapper-build-template-and-signing.md`
  summary: `infra/android/build.ts`'s per-slug scratch build directory (`dist/.work/<slug>`) has no concurrency guard — two simultaneous invocations for the same slug would race on `rmSync`/`cpSync`/the Bubblewrap build.
  evidence: Edge Case Hunter flagged this. Not a real risk for today's single-operator manual-CLI scope (spec explicitly runs it "by hand with explicit arguments"); becomes relevant only if a future story (20.4) ever triggers concurrent tenant builds from an automated pipeline.

## Deferred from: code review of story-20.2 (2026-09-04)

- source_spec: `_bmad-output/implementation-artifacts/spec-20-2-per-tenant-digital-asset-link-verification.md`
  summary: `apps/web/app/.well-known/assetlinks.json`'s unauthenticated reachability (AC3 — it must never be redirected to `/sign-in`) is only verified manually (a one-time live `curl`), not by an automated regression test that routes the request through `proxy.ts` itself.
  evidence: Verification Gap reviewer confirmed via repo-wide search — no `*.test.ts` exists anywhere for `proxy.ts`, and no `e2e/specs/` file covers `/.well-known/`; `route.test.ts` calls `GET()` directly, bypassing `proxy.ts`'s matcher entirely. This exact failure class already happened once for `/icons` in Story 20.1 (a dot-less path fell through the matcher, hit the session-cookie guard, got redirected to `/sign-in` HTML) — a future edit to `proxy.ts`'s matcher regex could silently reintroduce it for this route with nothing catching it. The spec deliberately scoped this to a manual check (mirroring 20.1's own precedent for parts without existing test infra); an automated equivalent — either a lightweight direct test of the exported `proxy` function with a constructed `NextRequest`, or a Playwright e2e spec following the existing `e2e/specs/smoke/` pattern — would close the gap.

## Deferred from: code review of story-20.3 (2026-09-04)

- source_spec: `_bmad-output/implementation-artifacts/spec-20-3-tenant-branded-apk-generation.md`
  summary: `infra/provisioning/provision.ts`'s `loadTenantConfig` does a bare `JSON.parse(raw) as TenantConfig` with no runtime schema validation, so a tenant JSON missing/misformatting the now-required `androidPackageId` (or any other field) would type-check fine and only fail once Story 20.4 actually starts reading it.
  evidence: Blind Hunter review confirmed by reading `provision.ts` — the type assertion is compile-time only, and this is a pre-existing pattern for every field on `TenantConfig`, not introduced by this story. Not fixed here (out of this story's data-only scope), but worth a deliberate follow-up (e.g. a Zod schema) once Story 20.4 actually depends on these values being well-formed.

## Deferred from: code review of story-20.4 (2026-09-04)

- source_spec: `_bmad-output/implementation-artifacts/spec-20-4-provisioning-pipeline-integration.md`
  summary: `infra/android/build.ts`'s first-ever run on a machine without a warmed `~/.bubblewrap/` SDK cache prompts interactively to download the ~1GB managed Android SDK — fine for a human running it by hand (its original 20.1 design), but `buildAndroidApk` now invokes it as one step in what's meant to become an unattended, scripted provisioning pipeline (and eventually CI). No non-interactive/pre-accept flag or SDK pre-warm step exists yet to prevent a fresh runner from hanging on that prompt.
  evidence: Blind Hunter review confirmed by reading `infra/android/README.md`'s own "first `pnpm android:build` run... prompts once" documentation against `build.ts`'s doc-comment claim that `update`/`build` are non-interactive — the SDK-acquisition prompt is a separate interactive point neither doc-comment addresses. Not fixed here: the right fix (pre-warmed CI image, a Bubblewrap non-interactive SDK-accept flag if one exists, or an explicit pre-flight check) is a CI/deployment-infrastructure decision outside this story's scope, and not yet reachable anyway since `createVercelProject` still throws before `buildAndroidApk` ever runs.
- source_spec: `_bmad-output/implementation-artifacts/spec-20-4-provisioning-pipeline-integration.md`
  summary: `buildAndroidApk`'s spawned subprocess (and, more broadly, every `await` in `provision.ts`'s `main()`) has no timeout — a hung Gradle build, SDK download, or future Vercel/Neon API call blocks the whole provisioning run indefinitely with no diagnostic beyond whatever inherited stdio already printed.
  evidence: Both Blind Hunter and Edge Case Hunter independently flagged the missing timeout on the Android build step specifically. Not fixed here as a one-off bolt-on: picking a sane timeout requires knowing real build durations (first-run SDK download alone can take several minutes) and the same gap exists — untested — on every other step in `main()`, so it reads as a cross-cutting `main()`-level concern better solved once, not per-step.
- source_spec: `_bmad-output/implementation-artifacts/spec-20-4-provisioning-pipeline-integration.md`
  summary: No unit tests for the new pure orchestration logic in `resolveTsxBin()`/`buildAndroidApk()` (package.json bin-shape validation branch, exit-code/signal branches) — mockable via `child_process.spawn`, unlike the other provisioning steps which genuinely can't be tested without live provider accounts.
  evidence: Blind Hunter flagged this; Verification Gap reviewer independently confirmed via repo-wide search that `infra/` has zero test coverage anywhere (outside `pnpm-workspace.yaml`, outside `turbo run test`, outside `.github/workflows/ci.yml`) — this is the same pre-existing no-CI-coverage convention already deferred once for `infra/android/build.ts`'s own pure logic in story 20.1's review, not newly introduced.
- source_spec: `_bmad-output/implementation-artifacts/spec-20-4-provisioning-pipeline-integration.md`
  summary: `buildAndroidApk` runs immediately after `createVercelProject`, but nothing in the pipeline actually confirms the tenant's Vercel deployment has finished building and its domain/TLS is live before fetching `--manifest-url` — the README's "from the now-fetchable manifest" framing is aspirational until `createVercelProject` is implemented for real.
  evidence: Blind Hunter review confirmed `createVercelProject`'s own TODO comment only promises "create project... set env vars," not triggering or waiting for a deployment. Not addressable now — the actual behavior (whether Vercel's API makes a domain live synchronously on project creation, or needs an explicit deploy+poll step) depends entirely on `createVercelProject`'s own not-yet-written implementation, which is explicitly out of this story's scope (its own TODO, untouched here). Whoever implements `createVercelProject` for real should revisit this ordering.

## Deferred from: code review of story-20.6 (2026-09-04)

- source_spec: `infra/android/DISTRIBUTION.md`
  summary: No record-keeping step in the distribution runbook — nothing logs which APK build/slug was sent to which tenant contact and when, needed for support triage ("which build does this client have installed?") and for knowing when a client is overdue for a rebuild-triggered redistribution.
  evidence: Blind Hunter review flagged this. Not fixed here — no existing precedent for this kind of tracking log anywhere else in this repo's manual runbooks (`infra/provisioning/README.md` doesn't track who was provisioned when either), so this would be introducing a new operational convention rather than following an established one. Worth a deliberate decision (a shared spreadsheet, a field in `infra/tenants/<slug>.json`, etc.) rather than an ad hoc addition to this file.
- source_spec: `infra/android/DISTRIBUTION.md`
  summary: The runbook only documents sending the APK to a Tenant's single `contact.phone`/`contact.email`, not how additional Supervisors/Owners on the same Tenant's field team each get the app on their own phone.
  evidence: Blind Hunter review flagged this. Not fixed here — unclear whether individual Team Members' phone numbers are reliably available to whoever's distributing (the tenant config only has one `contact` block; per-Team-Member contact info lives in the app's own Team Members feature, a different system) or whether the intended flow is the primary contact relaying the file internally. Needs a product decision, not a doc guess.
- source_spec: `infra/android/DISTRIBUTION.md`
  summary: No guidance for the shared-signing-key-rotation/compromise scenario — `infra/android/README.md` already notes losing the keystore means no tenant app can ever receive a signed update again, but neither doc says what to actually do (client-side uninstall/reinstall across every affected tenant?) if the key needs to be actively rotated.
  evidence: Blind Hunter review flagged this. Not fixed here — this is a bigger topic (incident response for a compromised shared secret affecting every tenant simultaneously) than a distribution runbook's scope; it deserves its own document once/if it's prioritized, not a bolted-on paragraph here.

## Deferred from: code review of story-21.1 (2026-09-06)

- source_spec: `_bmad-output/implementation-artifacts/spec-21-1-web-push-notifications.md`
  summary: `isPendingTerms()` (a predicate over one row) and `countDraftPendingTerms()` (the equivalent Prisma `where` clause) express the same five "pending terms" conditions independently, by hand, with a comment promising to keep them in sync.
  evidence: Blind Hunter + Verification Gap both flagged this. Not merged into one implementation — that would force `countDraftPendingTerms()`'s DB-side `COUNT` into an in-memory filter (fetch every Draft row, filter in JS), a real performance regression as Site Contracts grow. Instead added a regression test (`site-contracts.service.spec.ts`) that replicates the where-clause in plain JS and cross-checks it against `isPendingTerms()` across a representative table of shapes — genuine drift between the two now fails a test immediately.
- source_spec: `_bmad-output/implementation-artifacts/spec-21-1-web-push-notifications.md`
  summary: No e2e/Playwright coverage of the push subscribe/unsubscribe/receive flow — the project's own convention is at least one create-and-verify smoke test per new module surface.
  evidence: Blind Hunter + Verification Gap both flagged this. Real E2E proof was already done manually once this session (a persistent, non-incognito Chromium context — `browser.newContext()` is always incognito and Chrome disables the Push API there; headless also always reports `Notification.permission` as `denied`). A permanent automated version needs that same non-standard Playwright harness plus a realistic ~90s timeout for first-time FCM channel registration on a fresh profile — real effort, not a quick add; deferred rather than rushed.
- source_spec: `_bmad-output/implementation-artifacts/spec-21-1-web-push-notifications.md`
  summary: No component test for the sidebar's push UI states (enable/on/blocked/disabled-while-subscribing) in `app-shell.test.tsx`.
  evidence: Blind Hunter + Verification Gap both flagged this. The underlying hook (`use-push-notifications.ts`) has full unit coverage including the exact `subscribed` vs `permission` bug found during manual E2E testing; the sidebar wiring itself (`app-shell.tsx`) is comparatively low-risk (conditional rendering + two callback wirings) and `app-shell.test.tsx` doesn't currently mock `usePushNotifications` at all — deferred as lower priority than the trigger-logic gaps that were fixed.
- source_spec: `_bmad-output/implementation-artifacts/spec-21-1-web-push-notifications.md`
  summary: `PushSubscription` has no `updatedAt`/`lastSeenAt` and no proactive pruning mechanism beyond reactive 404/410 cleanup on an actual failed send — a subscription that's simply abandoned (device lost/reset without ever unsubscribing, or a Supervisor whose role never happens to trigger an Owner-targeted event) can sit forever.
  evidence: Blind Hunter flagged this. Low urgency at current single-tenant, small-user-count scale; a scheduled cleanup job is real future work, not a defect today.
- source_spec: `_bmad-output/implementation-artifacts/spec-21-1-web-push-notifications.md`
  summary: The service worker's `showNotification` call uses the same full-color app icon for both `icon` and `badge` — a `badge` is meant to be a small monochrome silhouette for the Android status bar, and no such asset exists in this diff.
  evidence: Blind Hunter flagged this. Needs a real design asset to be created, not a code fix — out of scope for this review pass.

## Deferred from: multi-goal split of Help & Guides annotated-screenshot intent (2026-09-06)

- source_spec: none
  summary: The Guide screenshot pipeline — an automated Playwright script that captures a real mobile+desktop screenshot per guide step (drawing the pulsing `accent-teal-700` highlight ring around the target field/button by CSS selector), plus wiring the captured images into `/help/[guideId]` with auto-device-match display. Fully specced in the UX spines (EXPERIENCE.md "Guide screenshot pipeline" row + DESIGN.md's Guide step annotation spec and ambient-pulse Do's/Don'ts carve-out, both updated 2026-09-06; approved mock at `ux-designs/ux-AzentisFieldOS-2026-08-12/mockups/25-help-guide-annotated-screenshot.html`).
  evidence: Split from the combined "fix guide content + build screenshot pipeline" intent — the two are independently shippable (complete guide text is valuable with no screenshots; the pipeline's display logic doesn't depend on which guides have complete steps), and the pipeline is real new build tooling (capture script, per-step selector/route data, image storage strategy, responsive rendering) versus the content fix's pure data edits to `packages/shared/src/content/help-content.ts`. Content fix shipped first; guide step data should gain its per-step target-selector/route fields when this pipeline lands.

## Deferred from: code review of spec-help-guide-screenshot-pipeline (2026-09-06)

- source_spec: `_bmad-output/implementation-artifacts/spec-help-guide-screenshot-pipeline.md`
  summary: 16 pre-existing `pnpm test:e2e` failures, unrelated to this diff, fall into two root causes that both predate this work — real bugs in the existing e2e suite's own specs, not the app.
  evidence: Surfaced while verifying the screenshot pipeline didn't disturb the e2e harness (it didn't — this diff added only new sibling files, zero existing e2e spec/fixture/config touched, confirmed via `git diff --stat` against `baseline_commit`). Root cause 1 (8 failures — `owner-dashboard-and-pricing.spec.ts:24`, `supervisor-daily-flow.spec.ts:78`, and their dependent assertions): `.selectOption()` called on the Purchase form's Vendor field, which is a Base UI `ComboboxField`, not a native `<select>` — confirmed via `git show c07ab1b:apps/web/app/(app)/movements/purchases/purchase-form.tsx`, i.e. already true before this session's baseline commit. Root cause 2 (8 failures — `sites.spec.ts:14`, `vendors.spec.ts:13`, `corrections.spec.ts`, `smoke/expenses.spec.ts`, `smoke/machinery-vehicles.spec.ts`, `smoke/materials.spec.ts`, `smoke/rmc.spec.ts`, `smoke/team.spec.ts`, `smoke/reports-and-settings.spec.ts`, and several `supervisor-daily-flow.spec.ts` cases): `page.getByText(name)` matches both the CSS-hidden desktop table row and the visible mobile-card copy of the same content once a list adopts `mobileCard` — the exact gotcha AGENTS.md already documents under the e2e section, with the documented fix (`visibleText()` from `e2e/fixtures/ui.ts`) never applied to these specs. Fix: update every listed spec to use `visibleText()` instead of `getByText()` for DataTable row assertions, and replace the two stale `.selectOption()` calls with the `pickCombobox()` helper already used elsewhere in the suite.
- source_spec: `_bmad-output/implementation-artifacts/spec-help-guide-screenshot-pipeline.md`
  summary: `e2e/help-screens/capture.spec.ts`'s `writeOutputs()` deletes `apps/web/public/help-screens/` (`fs.rmSync`) before writing the new images and manifest, rather than writing to a temp location and renaming into place atomically.
  evidence: Edge Case Hunter flagged this. Not fixed here — real only if the process is killed mid-write (a crash or Ctrl+C between the `rmSync` and the final `fs.writeFileSync(MANIFEST_PATH, ...)`), and the pipeline is a manually-run, human-supervised script whose output is reviewed via `git status`/`git diff` before ever being committed — an interrupted run leaves an obviously-incomplete working tree a human would notice, not a silent production risk. Worth an atomic temp-dir-then-rename write if this script is ever run unattended (e.g. from CI or a provisioning step).
- source_spec: `_bmad-output/implementation-artifacts/spec-help-guide-screenshot-pipeline.md`
  summary: The `<img onError>` fallback in `GuideStepScreenshot` (hiding the whole screenshot block for a step whose PNG is missing) may not reliably fire if the image fails to load before React's event handler attaches during hydration.
  evidence: Edge Case Hunter flagged this. Narrow window in practice — every image uses `loading="lazy"`, so the browser defers the actual fetch until the element nears the viewport, which happens well after hydration completes for any image not already on-screen at load. Worst case if it does race is a broken-image icon shown briefly rather than a crash or wrong content. A fully robust fix needs a client-side `naturalWidth`/`complete` check in a `useEffect`, meaningfully more code for a narrow edge case; not worth it for this pass.

## Deferred from: multi-goal split of "detail views should open in a side panel, not reload the page" intent (2026-09-07)

- source_spec: none
  summary: Extend the right-side detail-panel pattern (built for Vendor and Subcontractor in the current spec) to the remaining ~9 list-row-click locations that go through `DataTable`'s `rowHref` — Sites, Daily Activity/Reports (3 spots), and Team Members (2 spots) — so the whole app is consistent, not just the two entities the user explicitly reported.
  evidence: Root-cause investigation found this is one shared component (`packages/ui`'s `DataTable`) used identically by 11 call sites across 9 files, so the pattern generalizes cleanly, but retrofitting all 9 remaining pages is materially more surface area/review risk than the 2 explicitly-reported entities and isn't needed to prove out or ship the panel pattern itself. Independently shippable once the panel component exists.

## Deferred from: one-shot fix of DataTable/CorrectAction/StatTile/EntityChip full-page-reload bug (2026-09-07)

- source_spec: `_bmad-output/implementation-artifacts/spec-fix-shared-link-components-hard-reload.md`
  summary: `DataTable`'s desktop row rendering wraps each visible column in its own `<Link>` (N links per row) instead of one stretched link covering the whole row, the way `DataTableCardList`'s mobile card already does.
  evidence: Blind Hunter review flagged this. Pre-existing structure (the old code was one `<a>` per column too), but now that each is a real `next/link` `Link`, collapsing to one link per row would also remove N-1 duplicate "link" announcements per row for screen readers. Not fixed here — doing it safely inside real `<table>`/`<tr>`/`<td>` markup (not a div-based card) needs `position: relative` on `<tr>`, which has known cross-browser table-layout quirks, so it's a real layout-risk change, not a trivial patch. `prefetch={false}` was added instead, which removes the actual regression this session introduced (N background prefetch requests per row); the DOM/a11y consolidation remains a separate, pre-existing-shape improvement.
- source_spec: `_bmad-output/implementation-artifacts/spec-fix-shared-link-components-hard-reload.md`
  summary: `packages/ui`'s `next` dependency (moved to `peerDependencies`, pinned to `apps/web`'s exact `16.3.0`) still resolves as two separate installs in `pnpm-lock.yaml` (with two copies of `next`'s optional `sharp`/`@img/sharp-*` native binaries), because `packages/ui`'s own `devDependencies` need a local `next` for `tsc`/`vitest`, and `packages/ui` pins `"@types/node": "^22"` while `apps/web` pins `"^20"` — pnpm resolves a distinct peer-context copy per differing nearby peer version.
  evidence: Blind Hunter review flagged this; confirmed directly via `pnpm-lock.yaml` (two `next@16.3.0(...)` snapshot entries under the `snapshots:` section, differing only in the `@types/node` peer — a third, unparameterized `next@16.3.0:` line also exists but is just the shared package-metadata declaration, not a separate resolved install). The `@types/node` mismatch itself pre-dates this session (already `^22` in `packages/ui` vs `^20` in `apps/web` before this diff) and is shared with `apps/api` (`^22`) — aligning it is a repo-wide version decision out of scope for a link-behavior bug fix, not something to change silently as a side effect here.
- source_spec: `_bmad-output/implementation-artifacts/spec-fix-shared-link-components-hard-reload.md`
  summary: No test locks in the Link-vs-`<a>` behavior change itself, and `StatTile`/`EntityChip` specifically have zero click-based navigation coverage anywhere — `DataTable`/`CorrectAction` are click-tested end-to-end already (pre-existing specs, unaffected by this diff), but a regression to `StatTile`'s or `EntityChip`'s Link wiring would ship undetected.
  evidence: Blind Hunter flagged the general test gap; Acceptance Auditor and Verification Gap reviews (code-review pass, 2026-09-07) refined it after reading the actual specs. `e2e/specs/smoke/vendors.spec.ts` never clicks a row link at all (create-and-verify-visible only); `subcontractors.spec.ts`'s row-link click only asserts the resulting URL, which would pass identically against the old `<a>` hard-reload code — neither spec actually distinguishes a `next/link` transition from a hard reload, so the "verified via e2e" claim this entry originally made was overstated and has been corrected here. Separately, `e2e/specs/smoke/subcontractors.spec.ts:33` (via the Server-Component-rendered `subcontractors/[id]/page.tsx`) and `e2e/specs/corrections.spec.ts` (across both Client- and Server-Component list pages) do genuinely click-test `DataTable` rows and `CorrectAction` links end-to-end, including post-click URL/content assertions — a total navigation break in either component would be caught, just not the specific reload-vs-transition distinction. `StatTile` (Owner Dashboard's 7 `href`-bearing tiles, e.g. `owner-dashboard.tsx`) and `EntityChip` (the entire "recently viewed" feature, `recently-viewed-chips.tsx`) have no e2e coverage at all — their existing unit tests (`stat-tile.test.tsx`, `entity-chip.test.tsx`, `recently-viewed-chips.test.tsx`) only assert the static `href` attribute via RTL `render`, never a click — so a future regression to either component's navigation would pass every existing automated check.

## Deferred from: code review of spec-vendor-subcontractor-detail-side-panel (2026-09-07)

- source_spec: `_bmad-output/implementation-artifacts/spec-vendor-subcontractor-detail-side-panel.md`
  summary: `useVendorDetail`/`VendorDetailPanelContent` (vendors-list-client.tsx) and `useSubcontractorDetail`/`SubcontractorDetailPanelContent` (subcontractors-list-client.tsx) are ~140-line near-verbatim duplicates — same fetch/cancellation/"settled" logic, same state-union shapes, same skeleton/error/not-found JSX — with no shared abstraction extracted into `packages/ui` or a shared hook.
  evidence: Blind Hunter review confirmed via direct comparison of both files. Real DRY concern and drift risk (a future fix to the retry/cancellation logic is easy to apply to one copy and forget the other), but extracting a shared generic (e.g. `useEntityDetail<T>(id, path)`) touches both files non-trivially and wasn't required by the spec's stated boundaries — a deliberate follow-up refactor, not blocking correctness. Both copies now have full, symmetric test coverage (this review's patch pass added the previously-missing `subcontractors-list-client.test.tsx`), which at least makes future drift between them test-catchable.
- source_spec: `_bmad-output/implementation-artifacts/spec-vendor-subcontractor-detail-side-panel.md`
  summary: `useDetailPanelState`'s `router.replace` calls (on every row click and every panel close) pass no `{ scroll: false }`, unlike this being a much higher-frequency call site than `use-list-query-state.ts`'s own identically-shaped (also `scroll`-unguarded) filter/sort/search writes.
  evidence: Blind Hunter flagged this. Matches an already-shipped, unflagged pattern elsewhere in the codebase (`use-list-query-state.ts`), so not a novel regression, but this hook's calls fire far more often (every row click) — worth verifying in a real browser whether App Router's default scroll-restoration actually jolts a scrolled list back to top on a same-path query-only replace, and adding `{ scroll: false }` to both hooks together if so.
- source_spec: `_bmad-output/implementation-artifacts/spec-vendor-subcontractor-detail-side-panel.md`
  summary: Two of the spec's own six I/O-matrix acceptance criteria — loading `/vendors?vendorId=<id>` directly (panel reopens on mount) and the panel's "not found" state — are exercised only via mocked-fetch unit tests (`vendors-list-client.test.tsx`, `use-detail-panel-state.test.ts`), never against a real browser/API.
  evidence: Blind Hunter and Verification Gap both raised this independently. The unit coverage is real and exercises the same code paths, but a genuine end-to-end proof (real page load with the param already in the URL; a real 404 from apps/api) is stronger and currently missing from `e2e/specs/smoke/vendors.spec.ts`/`subcontractors.spec.ts`.
- source_spec: `_bmad-output/implementation-artifacts/spec-vendor-subcontractor-detail-side-panel.md`
  summary: No verification (test or manual) that keyboard focus returns to the row that opened the panel once it closes (Escape/backdrop/close button) — a WCAG-adjacent gap given the panel is opened via `onRowClick` interception rather than Base UI's own `Dialog.Trigger`, which is the mechanism Base UI's automatic focus-return normally expects.
  evidence: Blind Hunter flagged this. Base UI's `Dialog.Root` plausibly still restores focus to whatever element was focused when `open` became true regardless of *how* it became true (not documented one way or the other here), but this wasn't verified either way in a real browser — worth a targeted a11y check given this project's zero-tolerance WCAG AA policy (AGENTS.md).
- source_spec: `_bmad-output/implementation-artifacts/spec-vendor-subcontractor-detail-side-panel.md`
  summary: The e2e seed-script fix in this same diff (adding `Subcontractor`/`SiteContract`/`SubcontractorWorkEntry`/`SubcontractorPayment` to `ALL_TABLES`, previously never truncated between e2e runs) has no automated guard against the same class of drift recurring for a future new Prisma model — `ALL_TABLES` is a hand-maintained list with no test cross-checking it against `infra/prisma/schema.prisma`.
  evidence: Blind Hunter and Verification Gap both flagged this, and this diff's own bug (found while investigating an unrelated flaky test — 34+ accumulated Subcontractor rows leaking across every e2e run since Epic 18 shipped, silently pushing test data past pagination) is direct proof the drift already happened once undetected. A schema-vs-`ALL_TABLES` parity check (e.g. a small test that introspects Prisma's DMMF and diffs against the list) would catch this class of regression going forward; not added here to stay scoped to the panel feature's own fix.
- source_spec: `_bmad-output/implementation-artifacts/spec-vendor-subcontractor-detail-side-panel.md`
  summary: No test exercises clicking a second row while the panel is already open and showing a resolved first row (the `settled` derived-state guard's `result.id === id` check is what currently prevents showing the wrong entity's content) — code looks correct today, but nothing would catch a future regression that weakens the guard.
  evidence: Verification Gap review confirmed via reading both list-client files and every existing panel test (unit + e2e) — none change the panel's target id while it's already in the `"success"` state. Not a live bug; a coverage gap for a currently-correct code path.
- source_spec: `_bmad-output/implementation-artifacts/spec-vendor-subcontractor-detail-side-panel.md`
  summary: Two independent URL-writers on the same page — `useDetailPanelState` (panel open/close) and `useListQueryState` (search debounce) — each read their own `searchParams` snapshot and call `router.replace` independently; a row click racing a debounced search commit in the same tick could theoretically drop one of the two URL updates.
  evidence: Edge Case Hunter flagged this; no reproduction attempted, low real-world likelihood (requires near-simultaneous timing), and unifying both hooks into one atomic URL-writer is a larger architectural change than this feature's scope.
- source_spec: `_bmad-output/implementation-artifacts/spec-vendor-subcontractor-detail-side-panel.md`
  summary: The spec's own "Manual checks" item — confirming the panel doesn't visually collide with the mobile bottom quick-bar/`QuickAddSheet` on small viewports (both `fixed`-positioned) — was not actually performed (no real browser/viewport check), only reasoned about via reading CSS z-index values.
  evidence: Verification Gap review confirmed the z-index relationship looks correct on paper (`DetailPanel`'s backdrop/popup are `z-50`, `SupervisorQuickBar` is `z-30`, matching the existing mobile nav drawer's already-relied-upon stacking order) but flagged that this is a paper check, not a rendered one — the spec explicitly called for the latter.
- source_spec: `_bmad-output/implementation-artifacts/spec-owner-dashboard-command-center.md`
  summary: Web renders times/dates with hardcoded Asia/Kolkata while apps/api buckets days by the configurable APP_TIMEZONE — a non-IST tenant would see mismatched day windows vs displayed times.
  evidence: site-operations-table.tsx submittedTime and owner-dashboard.tsx heading both pin "Asia/Kolkata" (pre-existing page convention); apps/api/src/dashboard/local-day.ts resolves APP_TIMEZONE per deployment.
- source_spec: `_bmad-output/implementation-artifacts/spec-owner-dashboard-command-center.md`
  summary: Decide whether cross-Site financial dashboard aggregates (/dashboard/site-breakdown, /dashboard/trends, and pre-existing /dashboard/today expensesToday) should be @Roles(OWNER_ADMIN)-scoped or stay role-open.
  evidence: The dashboard controller has never carried @Roles (Supervisor Home consumes /dashboard/today), so the new endpoints match the existing posture — but EXPERIENCE.md's "trimming is de-emphasis, server guards are the boundary" line argues for an explicit product decision.
- source_spec: `_bmad-output/implementation-artifacts/spec-owner-dashboard-command-center.md`
  summary: supersededDsrIds() runs a full corrections scan per dashboard request (now 3 endpoints per page load) with an unbounded notIn list; window-scope it by reportDate or cache per-request.
  evidence: Pre-existing helper design in apps/api/src/common/superseded-dsrs.ts; getToday, getSiteBreakdown, and getTrends each await their own scan.
- source_spec: `_bmad-output/implementation-artifacts/spec-owner-dashboard-command-center.md`
  summary: Same-day activity recorded against a Site that is then soft-deleted vanishes from the site-breakdown table while remaining in the band's global counts, breaking reconciliation for that rare day.
  evidence: getSiteBreakdown buckets by the sitesService.list() roster; getToday counts purchases/consumption/expenses globally without a roster join.
- source_spec: `_bmad-output/implementation-artifacts/spec-dsr-drafts.md`
  summary: Dedicated "My Drafts" browsing/management screen — a cross-Site/date list of a supervisor's open Daily Report drafts with per-row resume/edit/discard, plus a Supervisor Home "continue draft" entry point.
  evidence: Split from the DSR-drafts backbone spec (2026-09-18) to keep that spec under the 1600-token guideline. The backbone still ships resume + Save Draft + Finalize + Discard directly on the DSR form for the picked (site,date); this deferred goal is the fuller list-management UI on top.
- source_spec: `_bmad-output/implementation-artifacts/spec-dsr-drafts.md`
  summary: Ability to REMOVE a photo already staged into a saved draft before finalizing (resumed draft photos arrive with file undefined and can be retried but not detached).
  evidence: Surfaced by the DSR-drafts adversarial review (2026-09-18); out of the backbone scope and a natural fit for the deferred "My Drafts" management UI.

## Deferred from: code review (2026-09-19)

- source_spec: (no spec — session review of user-mgmt/autosave/photo-feedback batch)
  summary: DSR autosave uses a single global localStorage slot (dsr-autosave-v1) — a second site/date session (deep-link or second tab) last-writer-wins the snapshot, and any durable save clears it even if the snapshot belonged to a different interrupted session.
  evidence: apps/web/lib/dsr-autosave.ts STORAGE_KEY; restore/clear call sites in apps/web/app/(app)/dsr/new/page.tsx. Accepted single-slot design for the one-phone-one-supervisor reality; keying by localDsrKey(siteId,reportDate) with most-recent-pick restore is the fuller design.
- source_spec: (no spec — session review)
  summary: No transactional guard against a tenant ending up with zero active OWNER_ADMIN accounts (only reachable via two admins deactivating each other concurrently; self-deactivation is already blocked).
  evidence: apps/api/src/users/users.service.ts setActive uses the array $transaction form; an active-owner count check needs an interactive transaction.
- source_spec: (no spec — session review)
  summary: No e2e spec for the deactivation lifecycle (deactivate → sign-in refused → refresh dies → reactivate → sign-in works); the guard-cache TTL behavior is only observable end-to-end.
  evidence: e2e/ harness and seeded Owner+Supervisor already exist; grep for isActive/deactivat under e2e/ returns nothing.
- source_spec: (no spec — session review)
  summary: Settings user list UX polish: group/sort deactivated accounts below active ones and de-emphasize/disable the role SelectField on deactivated rows.
  evidence: apps/web/app/(app)/settings/users-roles-section.tsx renders one flat DataTable with a status badge only.
- source_spec: (no spec — CRUD-reflection verification 2026-09-19)
  summary: MarkPaidButton's "Payment marked as paid" toast is unobservable — revalidatePath("/payments") re-renders the row as paid (unmounting the button) before its state.done effect can fire the toast. The row's badge flip is the only feedback (adequate, but the dead toast code should either work or go).
  evidence: apps/web/app/(app)/payments/mark-paid-button.tsx + payments/actions.ts markPaymentPaidAction; observed in the crud-reflection e2e authoring run (real browser).

## Deferred from: spec-dsr-activity-sync-detail-panel review (2026-09-20)

- source_spec: `_bmad-output/implementation-artifacts/spec-dsr-activity-sync-detail-panel.md`
  summary: Correcting a DSR Subcontractor entry to point at a different Site Contract than the one it was originally materialized against always throws (`createWorkEntry`'s correction guard requires `original.siteContractId === input.siteContractId`), with no client-side guardrail preventing the user from attempting it first.
  evidence: apps/api/src/subcontractors/work-entry-write.ts's correction branch. Fails safely (rejects the whole DSR correction, no data corruption) rather than corrupting data, but needs a product decision on whether re-linking to a different contract during a correction should be a supported flow at all before building a fix.
- source_spec: `_bmad-output/implementation-artifacts/spec-dsr-activity-sync-detail-panel.md`
  summary: Negative correction-delta rows (Waste Material tripCount, Subcontractor quantity) render on the DSR detail page with no visual indication they're signed deltas rather than absolute values (e.g. "Debris — -2 trips").
  evidence: apps/web/app/(app)/daily-activity/[id]/page.tsx's Waste Material/Subcontractor list sections. Needs a broader design pass on how correction-delta rows are displayed across the DSR detail page, not a one-line fix.

## Deferred from: "My Drafts" quick-resume (2026-09-21)

- source_spec: (no formal spec — built ad hoc against a deferred-work.md entry, then reviewed)
  summary: When a Supervisor has exactly one open DSR draft anywhere (any Site/date, however old), the Home hero permanently swaps from "Start Daily Report" to "Continue Daily Report" pointing at that draft, with no Discard/"start new" affordance visible on Home itself (Discard only renders in ResumeDraftsList, gated on 2+ drafts).
  evidence: apps/web/app/(app)/_components/supervisor-home.tsx's heroTask/singleDraft logic. A forgotten stale draft for one Site can silently hijack the one-tap daily-report flow for an unrelated Site with no escape hatch short of opening the form and finding Discard buried inside it. Fails safe (no data loss), but is a real workflow-friction gap — needs a product decision (e.g. a "Start new" secondary link, or only auto-swap for a recent draft) before fixing.
- source_spec: (same feature)
  summary: `listMyDrafts` ordering test (`dsr.service.integration.spec.ts`) uses a fixed 10ms `setTimeout` between two saveDraft calls to force distinct `updatedAt` values — a latent, low-probability flakiness source rather than a hard guarantee.
  evidence: apps/api/src/dsr/dsr.service.integration.spec.ts's listMyDrafts describe block. Low risk given Postgres TIMESTAMP(3) millisecond resolution, but not deterministic under CI load.

## Deferred from: multi-goal split of Daily Reports/Movement/Inventory request (2026-09-23)

- source_spec: none
  summary: Simplify/unify Material Purchase and Movement for Site Engineers — a single guided "Material → Quantity → From → To → Date → Notes" flow over the existing Vendor→Godown/Vendor→Site/Godown→Site/Site→Site/Site→Godown transactions, replacing today's 7 separate creation buttons/routes.
  evidence: Research confirmed all 5 flows already work end-to-end today on one unified Purchase/Movement model with shared stock math, and a Site Engineer already has API access to all of them (only pricing/bill-attach is Owner-only) — so this goal is UI/UX-only, independently shippable from the Daily Reports and Inventory-screen goals, and the user explicitly asked to skip it for this round to focus on the other two.

## Deferred from: code review of spec-inventory-availability-screen (2026-09-23)

- source_spec: `_bmad-output/implementation-artifacts/spec-inventory-availability-screen.md`
  summary: A Material with a `lowStockThreshold` set but literally zero `GodownStock`/`SiteStock` rows anywhere (never purchased/moved) can never appear as a row in the new unified Inventory table, even under the "Low Stock" quick filter, since the table only ever shows rows that exist in the merged Godown+Site data.
  evidence: `StockService.listInventory`'s LOW filter (`apps/api/src/inventory/stock.service.ts`) narrows the already-merged Godown/Site rows by `lowStockMaterialIds.has(row.materialId)` — a Material with no stock row at all contributes no row to merge in the first place. The pre-existing, unchanged Alerts/GapFlag section above the table (driven directly by `getLowStockMaterials()`) still surfaces such a Material correctly; only the new table itself can't show it as a row. Fixing this needs a product decision on whether to synthesize a zero-quantity placeholder row.
- source_spec: `_bmad-output/implementation-artifacts/spec-inventory-availability-screen.md`
  summary: The desktop table's "Location" and "Location Type" columns both read "Godown" for every Godown row (the true singleton Godown has no name of its own) — literal duplication, though Site rows don't have this problem (Location shows the Site's real name).
  evidence: `apps/web/app/(app)/inventory/inventory-list-client.tsx`'s `columns` array — `Location` cell is `row.locationType === "GODOWN" ? "Godown" : row.siteName`, `Location Type` cell is a Badge reading "Godown"/"Site". The original request's own example table implied a named Godown ("Main Godown"), which this app's schema doesn't have (Godown is an unnamed singleton) — both columns were explicitly requested, so this isn't a bug to silently drop a column for; needs a design call on how to de-duplicate the Godown case without removing either column.
- source_spec: `_bmad-output/implementation-artifacts/spec-inventory-availability-screen.md`
  summary: The mobile card view repeats the same "Godown" text twice for Godown rows — once in the primary line ("… · Godown"), once in a "Location Type: Godown" detail row — because `DataTableMobileCard`'s `omitHeaders` is a single static list applied to every row, so it can't omit "Location Type" only for the Godown subset (Site rows' detail row is not truly redundant, since their primary line shows the Site's name, not the word "Site").
  evidence: `apps/web/app/(app)/inventory/inventory-list-client.tsx`'s `mobileCard.omitHeaders` is `["Material", "Location"]`; adding "Location Type" would also hide it for Site rows where it's not redundant. Needs either a `DataTableMobileCard` API extension (per-row conditional omission) or accepting the minor Godown-only redundancy.
- source_spec: `_bmad-output/implementation-artifacts/spec-inventory-availability-screen.md`
  summary: `stock.service.spec.ts` has no test for `sort=updatedAt`, none asserting an unrecognized filter/sort value (e.g. `locationType=FOO`) falls back to "no filter"/default as the inline code comments claim, and none combining `stockLevel=LOW` with a `siteId`/`locationType=SITE` filter together. Separately, the pre-existing `page.test.tsx` assertion "never renders a bare warning badge in the Godown Stock table — AC #2's low-stock signal is the GapFlag alone" was removed with this change and has no replacement test against the new merged table (the underlying rule still holds — the new "Location Type" Badge is unrelated to low-stock signaling — but it's untested against the new component).
  evidence: Confirmed via `git diff` on `apps/api/src/inventory/stock.service.spec.ts` and `apps/web/app/(app)/inventory/page.test.tsx` — all real test-coverage gaps, not proven defects.
- source_spec: `_bmad-output/implementation-artifacts/spec-inventory-availability-screen.md`
  summary: `SiteInventoryReportsService.getInventoryReport` can now return the full, unbounded all-Sites `SiteStock` table when both `siteId` and `materialId` are omitted (previously it always returned `[]` in that case) — a new source of a potentially large payload composed into a report.
  evidence: `apps/api/src/reports/site-inventory-reports.service.ts`'s no-`siteId` branch now calls `this.stock.getAllSiteStock(materialId)` (was `Promise.resolve([])`). This matches the same call's already-unbounded, already-accepted behavior for `getGodownStock(materialId)` in the same object when `materialId` is also omitted — not a new class of risk, but a new occurrence of it on the Site Stock side; worth watching if report volume/stock-row count grows.
- source_spec: `_bmad-output/implementation-artifacts/spec-inventory-availability-screen.md`
  summary: The "Low Stock" quick filter can show a Site row with a high quantity, flagged only because its Material is under threshold at the Godown (by design, per this spec's own Design Notes) — but the UI has no caption/tooltip explaining this to the end user, who may read a seemingly well-stocked row under "Low Stock" as a bug.
  evidence: Confirmed by the Blind Hunter review layer; the underlying behavior is intentional and spec-approved, but the UI gives no indication of why — a real user-facing confusion risk, needs a product decision on messaging (e.g. a tooltip on the LOW badge/chip).

## Deferred from: multi-goal split of "Submitted Daily Reports" request (2026-09-23)

- source_spec: none
  summary: "Void" a submitted Daily Report — a new correction type that reverses all of a report's stock/ledger effects and marks it void, with a red confirmation warning naming the inventory impact, while keeping the full original content permanently in the database and audit trail (AD-9-compliant, user-approved semantics for "Delete").
  evidence: User-approved decision (this session) was to keep Void as its own scoped piece, split from the List+Edit work — it touches DsrService's correction/materialization/stock-reversal logic directly (higher risk, needs focused review on its own) and is independently shippable: the List+Edit work is useful without it.
- source_spec: none
  summary: Extend the global `AuditLog` (used by every audited entity in the app, not just Daily Reports) to capture structured before/after value snapshots per change, not just who/what-entity/when.
  evidence: User-approved decision (this session) to split this out — it's a platform-wide schema/interceptor change (`AuditLogInterceptor`, `AuditLog` model) that benefits every module, not a DSR-specific change, so it deserves its own focused spec and review rather than being bundled with DSR-specific List+Edit/Void work.

## Deferred from: code review of spec-daily-reports-list-and-edit (2026-09-23)

- source_spec: `_bmad-output/implementation-artifacts/spec-daily-reports-list-and-edit.md`
  summary: `getSubmissionChain`'s `MAX_HOPS = 100` safety valve truncates a corrupt/cyclic `correctsId` chain silently, with no log/warning — `submittedAt`/`versions` would be computed from a truncated chain with no signal to anyone.
  evidence: `apps/api/src/dsr/dsr-correction-chain.ts`'s `while (correctsId && hops < MAX_HOPS)` loop. Correction chains are expected to be short in practice (0-2 hops) and 100 is a generous ceiling — this is a defensive-only edge case, not a real-world limit, but worth a `logger.warn` if it's ever touched again.
- source_spec: `_bmad-output/implementation-artifacts/spec-daily-reports-list-and-edit.md`
  summary: `listAllSubmitted` calls the unscoped, full-table `supersededDsrIds(this.prisma)` on every request, then runs `getSubmissionChain`'s backward chain-walk once per row on the current page (N sequential extra round-trips) — a new, likely-frequently-visited list page compounding two separate not-yet-bottlenecked-but-unbounded read costs as correction volume grows.
  evidence: `apps/api/src/dsr/dsr.service.ts`'s `listAllSubmitted` (`supersededDsrIds` call, then `Promise.all(rows.map(...getSubmissionChain...))`). Not a proven regression today (correction chains are short, `supersededDsrIds` mirrors an already-accepted pattern used by `listByDate`/`listBySiteInRange`) — worth revisiting if this page's load time becomes a real complaint.
- source_spec: `_bmad-output/implementation-artifacts/spec-daily-reports-list-and-edit.md`
  summary: A narrow TOCTOU window exists between `listAllSubmitted`'s `supersededDsrIds()` read and its subsequent `findMany`/`count` calls — a report corrected in between could show a since-superseded version for one request.
  evidence: `apps/api/src/dsr/dsr.service.ts:1648-1694`. Matches the same class of already-accepted, low-severity TOCTOU gaps logged elsewhere in this codebase (e.g. the pending-pricing dashboard deep-link); the window is milliseconds and the consequence is a rare stale read, not data corruption.
- source_spec: `_bmad-output/implementation-artifacts/spec-daily-reports-list-and-edit.md`
  summary: The new history search (`q`) only matches Site name and Submitted By name, narrower than the global Search palette's DSR coverage (which also matches every free-text narrative field) — a user familiar with global search may expect the same breadth here.
  evidence: `apps/api/src/dsr/dsr.service.ts`'s `listAllSubmitted` vs `searchCandidates` in the same file. A deliberate scope choice per the spec (Site + Submitted By only), not a bug — flagged in case broader matching is wanted later.
- source_spec: `_bmad-output/implementation-artifacts/spec-daily-reports-list-and-edit.md`
  summary: "Submitted Time" has no `sortKey` on the history list (not sortable) while the neighboring "Last Updated" column is — no visual/textual cue explains why one timestamp is interactive and the other isn't.
  evidence: `apps/web/app/(app)/daily-activity/history-list-client.tsx`'s column definitions. Minor UX inconsistency; whether "Submitted Time" is even a meaningful independent sort axis (it equals "Last Updated" for the common never-edited case) is a product question, not a code defect.
- source_spec: `_bmad-output/implementation-artifacts/spec-daily-reports-list-and-edit.md`
  summary: The currently-viewed entry in the detail page's "Version history" card is still a clickable self-linking `<Link>` (styled bold + "(viewing)" text) rather than non-interactive text or marked `aria-current="page"`.
  evidence: `apps/web/app/(app)/daily-activity/[id]/page.tsx`'s Version history card. Minor accessibility/UX polish, not a functional defect.
- source_spec: `_bmad-output/implementation-artifacts/spec-daily-reports-list-and-edit.md`
  summary: The Site filter dropdown on `/daily-activity/history` is populated from `GET /sites`, which excludes soft-deleted Sites — a Daily Report submitted for a since-deleted Site still appears (unfiltered) in the list, but can no longer be isolated via the Site filter once that Site is gone from the picker.
  evidence: Matches the codebase's existing, already-accepted soft-delete convention (deleted rows stay in history but drop out of pickers) — same class of gap already logged for other soft-deleted-entity pickers elsewhere.
- source_spec: `_bmad-output/implementation-artifacts/spec-daily-reports-list-and-edit.md`
  summary: Only `e2e/specs/crud-reflection.spec.ts` was updated for the Correct→Edit copy change; other e2e specs (auth, corrections, supervisor-daily-flow, smoke) were not checked for stale "Correct"/"correction" Daily-Report copy.
  evidence: AGENTS.md already documents this exact gap recurring from the 2026-09-20 rename batch ("Site Supervisor"→"Site Engineer" etc.) — e2e specs have their own dedicated DB/server setup, outside this spec's normal verification scope.
- source_spec: `_bmad-output/implementation-artifacts/spec-daily-reports-list-and-edit.md`
  summary: `GET /dsr/history` accepts `siteId`/`from`/`to`/`page`/`pageSize`/`sort`/`order` as raw, unvalidated query strings, unlike sibling routes on the same controller (e.g. `getDraft`) which run their query through `ZodQueryValidationPipe`.
  evidence: Fails safe today (malformed input yields empty Prisma results, not a crash) — matches the broader unvalidated-`@Query()` convention already accepted repeatedly elsewhere in this codebase, but is a real inconsistency within this one controller worth a look.
- source_spec: `_bmad-output/implementation-artifacts/spec-daily-reports-list-and-edit.md`
  summary: The detail page's "Edit" affordance is a hand-rolled `Link`+icon+label rather than reusing the shared `CorrectAction` component the new history list uses for the identical action — a structural AD-5 duplication, though not a drop-in fix, since `CorrectAction` is icon-only (sized for a table row) while the detail page needs a labeled page-header action.
  evidence: `packages/ui/src/components/correct-action.tsx` (icon-only `Button`/`Link`, no labeled variant) vs `apps/web/app/(app)/daily-activity/[id]/page.tsx`'s own inline action. A real fix needs a small design decision (e.g. a labeled variant on `CorrectAction`, or accepting two purpose-built components) rather than a one-line patch.

## Deferred from: 2026-09-23 cross-module batch request (multi-goal split, chose to build goal 1 — Labour Payment week Sunday-Saturday — first)

- source_spec: none
  summary: Labour Attendance/Presenty — add separate Day and Night attendance handling per labourer per date, with Full Day/Half Day for each, so a labourer can have both a Day entry and a Night entry on the same date, and weekly payment summaries break out Day vs Night contribution.
  evidence: User-provided multi-goal request split at intent-clarification time. `DailyLabourAttendance` today has no shift/day-night/half-day dimension at all (one row per labourer/site/date, `attended` boolean only) — this is new schema + uniqueness-rule + payment-calc + UI work, the highest-effort item in the batch, deliberately deferred to its own focused pass.
- source_spec: none
  summary: Add a proper search (by name and category) wherever the Labour list is displayed, and introduce fixed Labour categories (Men, Women, Mistri).
  evidence: User-provided multi-goal request split at intent-clarification time. `DailyLabourer.category` is currently free text with no enum/lookup table — introducing fixed categories is a small schema/validation decision that should land together with (or right before) the DSR Labour dropdown goal below, since that goal depends on the same category list.
- source_spec: none
  summary: Daily Status Report → Labour section — replace the static/free-text labour list with a searchable dropdown backed by the Labour Payment module's `DailyLabourer` registry, plus an inline "+ Add Labour" affordance so newly added labourers are immediately available without leaving the DSR form.
  evidence: User-provided multi-goal request split at intent-clarification time. DSR's `labourEntries` is currently a denormalized free-text JSON array (2026-09-20 client-readiness batch, informational tagging only) with no link to `DailyLabourer` — wiring the two together is a real sync feature, coupled to the Labour-categories goal above, and independently shippable from the week-cycle fix.
- source_spec: none
  summary: Daily Status Report editing — allow changing Site and Date on an existing report, and allow viewing existing uploaded photos, removing individual photos, and uploading additional photos on the edit screen (without affecting unrelated attachments).
  evidence: User-provided multi-goal request split at intent-clarification time. Separate feature on the DSR edit/correction flow, architecturally unrelated to Labour Payments — own scope, own review.
- source_spec: none
  summary: Fix the ~15+ second delay before a newly created Material Category becomes available in the Material form's category dropdown, without requiring a hard refresh or page reload.
  evidence: User-provided multi-goal request split at intent-clarification time. Isolated frontend cache/revalidation bug, unrelated to Labour Payments — good candidate for a fast, low-risk follow-up pass since it's fully self-contained.
- source_spec: none
  summary: Fix the synchronization gap between Daily Status Report subcontractor entries and Site Details → Subcontractors, so a subcontractor added via a DSR becomes visible (and stays in sync) under Site Details → Subcontractors instead of living as an independent duplicate record.
  evidence: User-provided multi-goal request split at intent-clarification time. DSR's `subcontractorEntries` is free-text JSON, disconnected from the real `Subcontractor`/`SiteContract` master records used by Site Details — same shape of problem as the DSR Labour-dropdown goal above, but a separate module deserving its own spec/review.
- source_spec: none
  summary: Verify RMC Rate is actually optional end-to-end (user reports it currently blocks saving an RMC entry without a rate); if a concrete repro exists, fix the specific gap rather than re-implementing optionality that already appears to be in place.
  evidence: Investigation during intent-clarification (2026-09-23) found `RmcEntry.ratePerM3`/`totalAmount` already nullable in `infra/prisma/schema.prisma`, `packages/shared/src/schemas/rmc-entry.ts` already treats `ratePerM3` as optional, and `apps/web/app/(app)/rmc/rmc-form.tsx` already has no `required` attribute on that field with hint text "Optional — leave blank if pricing isn't known yet." Deferred as a verify-only follow-up rather than assumed-necessary new work.
- source_spec: none
  summary: Show entered Notes in the corresponding table/list view wherever Notes are captured across the app (Labour, Attendance, DSR, Inventory, RMC, Subcontractors, etc.), with a shortened preview + expand/tooltip for long notes.
  evidence: User-provided multi-goal request split at intent-clarification time. Cross-cutting sweep across many existing Notes fields with no architectural coupling to any other goal — low risk but touches many files; can ride along with whichever future goal touches a given module, or be done as its own dedicated pass.

## Deferred from: code review of spec-material-category-immediate-availability (2026-09-23)

- source_spec: `_bmad-output/implementation-artifacts/spec-material-category-immediate-availability.md`
  summary: The Edit Material form (`/materials/[id]/edit`) also fetches `/material-categories` for its Category picker but isn't revalidated by any Category or Unit master-data action (`categories/actions.ts`'s `revalidateCategoryPaths()`, `units/actions.ts`'s `revalidateUnitPaths()`) — a newly created/renamed/disabled Category or Unit can stay stale there. Pre-existing gap in both files, not introduced by this change; no shared `/materials` layout exists to close it with a single `revalidatePath(..., "layout")` call, so fixing it needs its own scoped change (e.g. adding the path to both existing helpers, or a cross-cutting revalidation approach).
  evidence: Blind-hunter review of the /materials/categories revalidation fix (2026-09-23) found this gap; confirmed by inspecting `units/actions.ts`'s `revalidateUnitPaths()`, which has the identical gap, and confirmed no `apps/web/app/(app)/materials/layout.tsx` exists.
- source_spec: `_bmad-output/implementation-artifacts/spec-material-category-immediate-availability.md`
  summary: `apps/web/app/(app)/inventory/page.tsx`'s Category filter dropdown and `apps/web/app/(app)/settings/page.tsx`'s Material Categories summary card both fetch `/material-categories` (the inventory one with `cache: "no-store"`, same staleness mechanism this fix addressed) but neither is revalidated by any Category/Unit action.
  evidence: Blind-hunter review of the /materials/categories revalidation fix (2026-09-23). Lower severity than the Material-form case (a stale filter/count card, not a blocked data-entry flow) — same root cause class, different route trees, deliberately left out of this scoped fix.
- source_spec: `_bmad-output/implementation-artifacts/spec-material-category-immediate-availability.md`
  summary: An untracked, non-gitignored video artifact (`e2e/help-screens/preview-video/create-site.webm`) is sitting in the repo worktree, and `.gitignore` has no entry for help-screens video-capture output at all (neither the capture script's own default `.video-preview-tmp` directory nor a `*.webm` pattern), so running `e2e/help-screens/capture-video.spec.ts` normally leaves an untracked artifact in `git status` for any contributor.
  evidence: Blind-hunter review (2026-09-23) noticed this pre-existing untracked directory (present since before this session started, unrelated to the Category fix) while inspecting `git status`. Real and confirmed, but entirely unrelated in topic — logged rather than fixed inline.

## Deferred from: spec-labour-attendance-day-night-shift (2026-09-23, token-size split)

- source_spec: `_bmad-output/implementation-artifacts/spec-labour-attendance-day-night-shift.md`
  summary: Weekly Payment ledger Day/Night earnings breakdown — add nullable `dayEarned`/`nightEarned` columns to `DailyLabourWeeklyPayment` (server-computed at settlement, mirroring `totalEarned`, legacy rows stay null), plus a ledger UI column showing the split, plus a live Day/Night sub-total on the in-progress week view.
  evidence: The core Day/Night + Half Day attendance-recording spec exceeded the 1600-token scope budget (~2300 tokens). This piece is genuinely separable — `totalEarned` already sums correctly across mixed Day/Night rows with zero changes once `shift` exists on `DailyLabourAttendance`; this is purely an additive display/reporting enhancement on top of the core recording capability, not a blocker for it. User chose to split rather than accept the oversized spec.

## Deferred from: code review of spec-labour-attendance-day-night-shift (2026-09-23)

- source_spec: `_bmad-output/implementation-artifacts/spec-labour-attendance-day-night-shift.md`
  summary: The new duplicate-shift guard in `DailyLabourAttendanceService.create()` is a check-then-act race (a `findFirst` runs before, not inside, the `$transaction`) — two near-simultaneous submissions for the same labourer/date/shift (double-tap, two tabs) can both pass the check and create two "current" rows, silently double-paying. No DB-level unique/partial index enforces the invariant either.
  evidence: Independently raised by two review layers (blind-hunter, edge-case-hunter). Matches this module's existing, already-accepted architecture — no DB-level uniqueness constraint backs ANY of this module's correction/current-row invariants today (confirmed: `current-labour-attendance.ts` is purely app-level). Fixing needs either a DB constraint or restructuring the check inside the transaction — a bigger change than this scoped feature, and the module currently has zero production data so the real-world exposure is low.
- source_spec: `_bmad-output/implementation-artifacts/spec-labour-attendance-day-night-shift.md`
  summary: No index exists on `DailyLabourAttendance(labourerId, workDate, shift)` even though the new duplicate-shift guard and correction-match logic query exactly that shape.
  evidence: Raised by blind-hunter. Low urgency given the module has no production data yet; worth adding once real query volume exists.
- source_spec: `_bmad-output/implementation-artifacts/spec-labour-attendance-day-night-shift.md`
  summary: A correction to an Attendance row validates that `labourerId` and (after this patch) `shift` match the original, but never validates `siteId` — a correction could silently change which Site an attendance row is attributed to.
  evidence: Raised by blind-hunter, confirmed pre-existing (the `siteId` check was never present even before `shift` was added) — not newly introduced by this feature, just newly adjacent to the new shift check. Whether changing Site via a correction is intentional or an oversight needs a product decision.
- source_spec: `_bmad-output/implementation-artifacts/spec-labour-attendance-day-night-shift.md`
  summary: There is no UI path to correct an existing Attendance row (`correctsId` is supported end-to-end at the schema/service level but never wired from any form) — clicking an already-filled Day/Night calendar cell always opens a blank create-mode modal, which (after this feature) now hits the duplicate-shift guard and is rejected with no way to actually perform the suggested "correct it instead."
  evidence: Raised by blind-hunter and verification-gap independently. This was already a known, explicitly-deferred gap (a prior session's memory notes "no correction UI for individual Attendance/Advance rows, just API support") — this feature's new duplicate-shift guard makes the dead end more visible/reachable (a common click on an already-recorded cell now always fails) rather than introducing the gap. Building a correction UI is a real, separate, non-trivial feature (needs prefill of the existing row's values) — out of scope for this spec per its own explicit "Never" boundary.

## Deferred from: code review of spec-labour-search-and-fixed-categories (2026-09-23)

- source_spec: `_bmad-output/implementation-artifacts/spec-labour-search-and-fixed-categories.md`
  summary: No `pg_trgm` GIN index exists for `DailyLabourer.name`/`category`, even though `list()`'s search runs the same leading-wildcard `contains`/insensitive query pattern that Site/Material/Vendor/TeamMember/Subcontractor/RmcEntry/Expense/Payment all already got dedicated GIN trigram indexes for.
  evidence: Blind-hunter review (2026-09-23). Zero urgency today (module has no production data), but the query pattern already matches every other indexed entity — worth adding via the AGENTS.md-documented hand-written-migration path (GIN indexes are a Prisma-undeclared-object case, `db:migrate:deploy` only) once real row counts exist.
- source_spec: `_bmad-output/implementation-artifacts/spec-labour-search-and-fixed-categories.md`
  summary: `DailyLabourer` is not included in `SearchService`'s global-search entity fan-out, unlike its master-data peers (Vendor, TeamMember, Subcontractor, Material) — a Labourer can't be found from the app-wide search palette.
  evidence: Blind-hunter review (2026-09-23). Real UX gap, but touches a different service/module — separate scoped change.
- source_spec: `_bmad-output/implementation-artifacts/spec-labour-search-and-fixed-categories.md`
  summary: There is no `PATCH`/edit endpoint for `DailyLabourer` at all (only `POST create` and `GET`) — any row created with a legacy free-text category (from before the fixed-set change) can never be corrected through the UI, and there's no way to edit `name`/`defaultPerDayAmount`/`isActive` either.
  evidence: Blind-hunter review (2026-09-23). Pre-existing gap (no edit endpoint existed before category became fixed either) — a real, separate "edit Labourer master data" feature, not caused by this change.
- source_spec: `_bmad-output/implementation-artifacts/spec-labour-search-and-fixed-categories.md`
  summary: Minor UX enhancements identified but out of the original ask's scope: Category could render as a `Badge` (small closed set, like Vendors' `materialsSupplied`) instead of plain text; a one-click Category filter (segmented buttons or dropdown) could complement the free-text search given only 3 possible values; sortable fields could include `defaultPerDayAmount`/`outstandingAdvanceBalance` (real Decimal columns, directly sortable, arguably more useful than alphabetical for a weekly-settlement workflow); `isActive` is threaded through the API/types but never surfaced or filterable in the list.
  evidence: Blind-hunter review (2026-09-23). All genuine but additive polish beyond what was asked ("proper Search option," "search by name and category") — not bugs, no functional risk from leaving them.

## Deferred from: code review of spec-dsr-labour-dropdown (2026-09-24)

- source_spec: `_bmad-output/implementation-artifacts/spec-dsr-labour-dropdown.md`
  summary: The mobile DSR form's draft-resume path (`apps/web/app/(app)/dsr/new/page.tsx`) silently drops legacy-shape `{category, men, women}` Labour rows on resume, with no warning shown to the user — the desktop correction page got a warning banner for the identical failure mode, mobile draft-resume did not.
  evidence: Confirmed via direct code read (same `.filter((l): l is {labourerId: string} => "labourerId" in l)` pattern, no banner). Lower severity than the correction-flow case: drafts are private, same-day, ephemeral (not permanent submitted history), and a user actively resuming a draft they just saved is more likely to notice a missing section than an Owner editing a months-old report. Real, but narrower risk window — only affects drafts saved in the transition window before this feature shipped.
- source_spec: `_bmad-output/implementation-artifacts/spec-dsr-labour-dropdown.md`
  summary: If a DSR Labour row references a `DailyLabourer` that's since been deactivated (`isActive: false`), the Edit form's `ComboboxField` shows that row as blank (its id isn't in the active-only `labourerOptions` list) with no indication why — confusing, though not actual data loss (the row's underlying `labourerId` state is untouched and still submits correctly if the user doesn't touch it).
  evidence: Verified directly: `ComboboxField`'s `selected = options.find(o => o.value === value) ?? null` is a display-only lookup; the row's own state isn't cleared. A real UX clarity gap (the row should probably show the deactivated Labourer's name, greyed out, rather than blank) but not a correctness/data-loss bug.
- source_spec: `_bmad-output/implementation-artifacts/spec-dsr-labour-dropdown.md`
  summary: The new legacy-row-drop warning banner (and the pre-existing `correctsId`/`correctedById` banners it was styled to match) has no `role="alert"`/`aria-live` region, so assistive-tech users aren't proactively notified when the page loads with it present.
  evidence: Confirmed this matches an existing, consistent pattern across every warning banner styled this way in `daily-activity/[id]/page.tsx` and `correct/page.tsx` — a broader, pre-existing accessibility polish opportunity spanning multiple banners, not a regression unique to this change.
