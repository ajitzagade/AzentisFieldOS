...
- source_spec: none
  ...
  evidence: Blind Hunter review flagged this; confirmed directly via `pnpm-lock.yaml` (two `next@16.3.0(...)` entries, differing only in the `@types/node` peer). The `@types/node` mismatch itself pre-dates this session (already `^22` in `packages/ui` vs `^20` in `apps/web` before this diff) and is shared with `apps/api` (`^22`) — aligning it is a repo-wide version decision out of scope for a link-behavior bug fix, not something to change silently as a side effect here.
- source_spec: none
  summary: No test locks in the actual behavior change (client-side transition vs. hard reload) in `data-table.test.tsx`/`correct-action.test.tsx`/`stat-tile.test.tsx`/`entity-chip.test.tsx`.
  evidence: Blind Hunter review flagged this. jsdom (this repo's component-test environment) cannot distinguish a `next/link` `Link` transition from a plain `<a>` hard navigation, and has no `IntersectionObserver` polyfill to exercise the `prefetch` code path either — a component test here would only assert the anchor still renders, which the pre-existing tests already do incidentally. Real behavior was instead verified end-to-end with real Playwright browser runs (`e2e/specs/smoke/vendors.spec.ts`, `e2e/specs/smoke/subcontractors.spec.ts`, both passing standalone) — the strongest verification this stack has available for this specific class of bug.
