...
## Change Log
- **2026-08-28 — critical HTTP-layer bug found by real-browser E2E and fixed:** this story's `@CurrentUser()` + method-level `@UsePipes(new ZodValidationPipe(...))` combination silently broke every affected route over real HTTP: Nest runs a method-level pipe against EVERY parameter, so the pipe validated the `AuthUser` object (and, on `correct`, the `:id` param string) against the *body* schema and returned `VALIDATION_FAILED` before the handler ever saw the request. Affected since this story landed: `POST /dsr`, `POST /dsr/:id/correct`, `POST /photos`, and (since 2026-08-28) `POST /consumption`. Neither the controller unit tests (call the method directly — pipes don't run) nor the service-level integration tests (no HTTP layer) could catch it; it surfaced only when a Playwright browser drove the real running stack. Fix: `ZodValidationPipe.transform` now no-ops for any parameter whose `metadata.type !== 'body'`. Lesson recorded for future stories: any new guard/pipe combination needs at least one real-HTTP smoke call.
