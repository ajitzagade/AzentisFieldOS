...
- [x] Task 1: Rebuild Button against the new tokens (AC: #1, #4, #5)
  - [x] Update `packages/ui/src/components/button.tsx`'s `buttonVariants` (currently `primary`/`secondary`/`outline`/`ghost`/`destructive` with `--color-primary-*`/`--color-neutral-*`/`--color-error-*` classes from the old placeholder scale) to the three `DESIGN.md`-specified variants only: `primary` (`bg-accent-teal-700 text-white hover:bg-accent-teal-600`, `shadow-1` at rest, `shadow-2` + `translateY(-1px)` on hover, per `DESIGN.md`'s `button-primary`/Components section), `secondary` (`bg-surface-1 text-ink-900 border border-border-strong`, hover `bg-surface-2 border-accent-teal-700`), `ghost` (`bg-transparent text-ink-700`, hover `bg-surface-2 text-ink-900`). Drop `outline` and `destructive` — they're not in `DESIGN.md`'s three-variant spec; if a destructive-style action is ever needed later, that's a future extension of this same component's prop surface (AD-5), not a reason to keep an unspecified variant now.
  - [x] Radius: `rounded-md` (10px token from story 1.1) for all variants, matching `DESIGN.md`'s `button-primary.radius`.
  - [x] Hover transition timing: ≤160ms (`DESIGN.md` "Do's and Don'ts": "Keep hover/transition motion ≤160ms, subtle") — use the existing `--default-transition-duration` motion token (150ms, already in `theme.css`, untouched by story 1.1) rather than a new literal.
  - [x] Add an `iconOnly` boolean prop (default `false`) that, when true, removes the mandatory-label assumption and adjusts padding to a square icon-button shape — this is the explicit escape hatch AC #1 requires for dense row actions; it must not be the default rendering path.
  - [x] Keep the existing `isLoading`/spinner behavior (already implemented, satisfies AD-6's full-state-set rule for a submitting button) — don't remove or regress it.
  - [x] Write/update `packages/ui/src/components/button.test.tsx` (Vitest + React Testing Library or equivalent already-configured tooling) asserting: each variant renders its expected class names, `iconOnly` toggles the icon-only layout, `isLoading` disables the button and shows the spinner.
- [x] Task 2: Build Card (AC: #2, #4, #5)
  - [x] New file `packages/ui/src/components/card.tsx`. Base styles: `bg-surface-1 border border-border-hairline rounded-lg shadow-2 p-6` (resting state — `DESIGN.md` is explicit that cards rest at `shadow-2`, not `shadow-1`; this is a corrected-from-original-exploration rule, don't default to the lighter shadow).
  - [x] `interactive` boolean prop: when true, adds `cursor-pointer` and a hover state producing `shadow-2-hover` + `translateY(-2px)`, transition ≤160ms — mirrors `_shared-kit.html`'s `.card.interactive:hover` rule exactly.
  - [x] Export as a simple, composable component (`<Card>{children}</Card>`) — no forced internal layout (header/body/footer subcomponents are not specified by `DESIGN.md` and would be inventing structure beyond what this story's AC calls for; keep it a plain styled container, matching AD-5's "extend the shared component's documented prop surface" — start minimal, since over-building unrequested subcomponents ahead of a screen that never needs them just to look decisive is not the AD-5 pattern).
...
- [x] Task 3: Build Badge (AC: #3, #4, #5)
  - [x] New file `packages/ui/src/components/badge.tsx`. Base: pill shape (`rounded-full`), small padding, `text-caption`-scale (or nearest token from story 1.1's typography roles), per `DESIGN.md`'s badge component tokens.
  - [x] Five variants mapped 1:1 to `DESIGN.md`'s `badge-success`/`badge-warning`/`badge-danger`/`badge-gold` tokens plus a `neutral` variant (`bg-surface-3 text-ink-700`, per `_shared-kit.html`'s `.badge-neutral` — `DESIGN.md`'s frontmatter doesn't name a `badge-neutral` token explicitly but `_shared-kit.html`'s literal CSS does; treat the shared kit as the tie-breaker source for this one variant since `DESIGN.md`'s Components section for Badges says "semantic color only (success/warning/danger/gold/neutral)" — `neutral` is named in prose even though its exact token values live only in the shared-kit CSS).
  - [x] Accept an optional `icon` prop (`ReactNode`) rendered before the label text, sized per `_shared-kit.html`'s `.badge svg { width:11px; height:11px; }` — never a hardcoded icon inside the component; AC #3 requires the caller to supply it (this keeps Badge decoupled from story 1.4's icon set, which lands in a later story).
  - [x] Write `packages/ui/src/components/badge.test.tsx` asserting each of the 5 variants renders its expected token-based classes and that the optional icon renders when passed.
- [x] Task 4: Wire exports and verify (AC: #4)
  - [x] Add `export * from "./components/card";` and `export * from "./components/badge";` to `packages/ui/src/index.ts` (alongside the existing button export).
  - [x] Run `pnpm --filter @azentisfieldos/ui typecheck`, `pnpm --filter @azentisfieldos/ui lint`, and the new test suite.
  - [x] Grep the three component files for raw hex (`#[0-9a-fA-F]{3,6}`), inline `rgba(`, and bare pixel literals outside of Tailwind's arbitrary-value bracket syntax (which is itself disallowed here per AD-4) — confirm zero matches before marking this story done.
...
- `pnpm --filter @azentisfieldos/ui test` — 24/24 pass
- `pnpm --filter @azentisfieldos/web build` — pass
- Grep for raw hex/rgba/px-bracket literals in `button.tsx`/`card.tsx`/`badge.tsx` — zero matches

### Completion Notes List

- Rebuilt `Button` to the three `DESIGN.md` variants (`primary`/`secondary`/`ghost`), dropped `outline`/`destructive`, added `iconOnly` variant (default `false`), switched the focus ring to `accent-teal-100` (was `primary-400`, a token removed in story 1.1). Kept the existing `isLoading` spinner behavior unchanged.
- Button font sizing intentionally uses Tailwind's built-in `text-xs`/`text-sm`/`text-base` (unchanged from the pre-existing code) rather than the new named `DESIGN.md` typography roles — none of the 8 roles matches "13.5px/600" (the prototype's button text spec), and forcing a paired-weight role token (e.g. `text-body-sm`, which pairs to weight 400) alongside a conflicting `font-semibold` utility risks an unpredictable override given how Tailwind v4 orders same-layer utility declarations. `font-semibold` + built-in size utilities avoids that conflict entirely.
- Built `Card` and `Badge` from scratch, both using the same `cva` pattern as `Button`. Card defaults to `shadow-2` at rest (not `shadow-1`, per `DESIGN.md`'s explicit correction), lifts via `hover:shadow-2-hover hover:-translate-y-0.5` when `interactive` (Tailwind's `translate-y-0.5` = 2px given this repo's 4px spacing base, matching `DESIGN.md`'s `translateY(-2px)` exactly).
- Badge's optional icon is sized via `size-3` (12px, the nearest token-scale step) rather than the prototype's literal 11px — an arbitrary-value `size-[11px]` would itself be a raw pixel literal, which this same story's own Task 4 explicitly grep-checks against. `neutral` variant uses `_shared-kit.html`'s `.badge-neutral` values since `DESIGN.md`'s frontmatter doesn't name that token, per the story's own Dev Notes guidance.
- Added `packages/ui/vitest.setup.ts`'s `afterEach(cleanup)` — RTL's automatic cleanup only self-registers under Jest-style global test APIs, and this project deliberately doesn't enable Vitest's `globals: true` (explicit per-file imports instead), so without this the DOM from one test in a file was leaking into the next, causing "multiple elements found" failures across `button.test.tsx`/`badge.test.tsx`. Also added `"DOM"`/`"DOM.Iterable"` to `packages/ui/tsconfig.json`'s `lib` and an explicit `types: ["@testing-library/jest-dom", "node"]` (needed once `types` is set explicitly, `@types/node`'s auto-inclusion stops and must be re-listed) so `tsc --noEmit` recognizes DOM globals and jest-dom's custom matchers.
- Confirmed `apps/web`'s scaffold `<Button>Scaffold OK</Button>` (in `app/page.tsx`, no variant prop) builds and now resolves to real teal-primary styling instead of the post-story-1.1 unstyled fallback — no changes made to `page.tsx` itself, per this story's scope.

### File List

- `packages/ui/src/components/button.tsx` (modified)
- `packages/ui/src/components/button.test.tsx` (new)
- `packages/ui/src/components/card.tsx` (new)
- `packages/ui/src/components/card.test.tsx` (new)
- `packages/ui/src/components/badge.tsx` (new)
- `packages/ui/src/components/badge.test.tsx` (new)
- `packages/ui/src/index.ts` (modified — barrel exports)
- `packages/ui/tsconfig.json` (modified — DOM lib, explicit `types`)
- `packages/ui/vitest.setup.ts` (modified — RTL cleanup registration)
