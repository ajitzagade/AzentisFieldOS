...
- **Daily Report (DSR)** — Also referred to internally as the Daily Site Report; the user-facing name is "Daily Report" everywhere in the app. The Site Engineer's once-daily structured log of a Site's activity, labour, material, RMC, machinery/vehicle use, expenses, issues, and photos.
- **Vendor** — A supplier of Material, RMC, or services, with purchase and payment history.
- **Subcontractor** — An external party engaged to perform a defined scope of work at a Site under commercial terms (fixed cost, per-trip, per-unit, or a custom rate). Distinct from a Team Member (internal labour pool, no commercial contract) and from a Vendor (supplies goods/RMC, not labour/work). Has contact details and, across Sites, a full history of Site Contracts and Subcontractor Payments. **Note:** not called "Contractor" — that term is already reserved for the tenant-owning business itself (see **Contractor / Company** above); using it for an external party would collide with every existing screen and document.
- **Site Contract** — A Subcontractor's engagement on one Site for one scope of work: rate type, agreed rate or fixed amount, start/end dates, and status (Draft, Active, Completed, Cancelled). One Subcontractor may hold multiple Site Contracts, on the same or different Sites.
- **Work Entry** — A recorded quantity of work completed against a Site Contract (e.g., trips run, pipes laid, units installed) on a given date. Append-only. Drives work-completed-vs-pending visibility and, for rate-based contracts, the amount payable.
- **Subcontractor Payment** — A recorded amount (advance or payment) paid to a Subcontractor against a Site Contract. Append-only. Net against the amount payable to produce an Outstanding Amount.
- **Expense** — Any recorded cost not otherwise captured as a Purchase (fuel, repairs, transport, misc.), tied to a Site and Category.
- **Role** — **Owner/Admin** (full tenant access, configuration, all sites) or **Site Engineer** (mobile DSR entry, scoped to whichever Site they're actively logging for, not permanently bound to one). These are the only two in-app roles. (Internally the role's enum value remains `SITE_SUPERVISOR`; "Site Engineer" is the display label only.)
