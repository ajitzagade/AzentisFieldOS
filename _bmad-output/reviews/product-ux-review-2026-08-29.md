...
**Tier 3 — output-changing bug fix, no writes/schema: ✅ SHIPPED 2026-08-30.** `financial-reports.service.ts` now (a) applies the shared superseded-DSR filter (`currentDsrRowsWhere`) to its RMC and Expense SUMs — matching `RmcService.statsThisMonth` / `ExpensesService.summary` — and (b) excludes restated Payments from the labour SUM (a Payment correction is a full restatement; only the chain tip is summed). Purchases stay deliberately unfiltered (standalone signed-delta corrections are meant to sum with their originals). Covered by 3 new unit tests; full API suite green including all 56 DB-integration tests against `azentisfieldos_test` (which needed the pending `add_site_description_and_challan_photo` migration applied — pre-existing drift, now fixed).
