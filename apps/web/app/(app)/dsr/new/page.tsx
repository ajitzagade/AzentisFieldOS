...
import { dsrEquipmentUsedSchema, type CreateDsrInput, type SaveDraftInput } from "@azentisfieldos/shared";
import {
  clearDsrAutosave,
  loadDsrAutosave,
...
// now (enforced by the onSubmit/Finalize guards), so only the Quantity
// value's own validity needs checking here.
...
// DailyLabourer registry. Revised 2026-09-24 (user-requested): a sibling
// Men/Women/Mistri headcount tally was restored alongside it on the SAME
// row — all four fields optional, a row is complete once at least one is
// set. Mirrors dsr-desktop-form.tsx's LabourRow exactly.
interface LabourRow {
  clientGeneratedId: string;
...
interface DraftResponse {
  ...
  // spec-dsr-labour-dropdown, revised 2026-09-24: a resumed draft may carry
  // either shape — the new {labourerId?, men?, women?, mistri?} shape or the
  // legacy free-text {category, men, women} shape (pre-dropdown). Only the
  // legacy shape is unrepresentable in this form now; it stays exactly as
  // saved on the draft's own draftContent JSON either way (never rewritten,
  // AD-9).
  labourEntries: (
    | { clientGeneratedId?: string; labourerId?: string; men?: number; women?: number; mistri?: number }
...
function NewDsrForm() {
  ...
  // Bugfix (2026-09-24): prefillFromDraft resolves each crew row's display
  // name from reference.teamMemberOptions at the moment a draft is resumed
  // — but reference data loads asynchronously (useDsrReferenceData's own
  // parallel fetch), so a resume that races ahead of it permanently showed
  // the "Crew member" fallback with no later correction, even once the
  // options DID arrive. This backfills any row still missing a name the
  // moment reference.teamMemberOptions becomes available.
  useEffect(() => {
    if (reference.teamMemberOptions.length === 0) return;
    // eslint-disable-next-line react-hooks/set-state-in-effect -- one-shot backfill guarded by the rows.some() check below; reference.teamMemberOptions only transitions empty->populated once per mount, so this can't cascade
    setCrew((rows) =>
      rows.some((r) => !r.name)
        ? rows.map((r) =>
            r.name ? r : { ...r, name: reference.teamMemberOptions.find((o) => o.value === r.teamMemberId)?.label },
          )
        : rows,
    );
  }, [reference.teamMemberOptions]);
  ...
  function prefillFromDraft(draft: DraftResponse) {
    ...
    // spec-dsr-labour-dropdown, revised 2026-09-24: only true legacy
    // {category, men, women} rows (pre-dropdown) aren't representable here —
    // they stay exactly as-is in draftContent (never rewritten, AD-9), just
    // not reproducible in this form.
    setLabourEntries(
      (draft.labourEntries ?? [])
        .filter(
          (l): l is { clientGeneratedId?: string; labourerId?: string; men?: number; women?: number; mistri?: number } =>
            !("category" in l),
        )
        .map((l) => ({
    ...
    return withClientGeneratedIds({
      ...
      subcontractorEntries: subcontractorEntries
        ...
        .map((s) => ({
          ...
          subcontractorId: s.subcontractorId!,
          // Revised 2026-09-24 (user-requested): Work note and Site
          // Contract are required for a real submission (blocked earlier by
          // the onSubmit/Finalize guards, which only run for that path —
          // buildPayload() is ALSO used by Save Draft, which must still
          // allow an in-progress, not-yet-filled-in row). `|| undefined`
          // converts an empty/unset value to a dropped key either way: the
          // strict createDsrSchema then genuinely can't be missing it
          // (guard already checked), while the relaxed saveDraftSchema
          // accepts the gap.
          workNote: s.workNote || undefined,
          siteContractId: s.siteContractId || undefined,
      ...
      // every other sub-record array here follows.
  ...
  async function doSubmit() {
    ...
    try {
      // buildPayload() is typed as the looser SaveDraftInput (Save Draft,
      // its other caller, must allow an incomplete Subcontractor row) — but
      // doSubmit() only ever runs after the onSubmit/Finalize guard above
      // already confirmed every Subcontractor row has its required Work
      // note + Site Contract, so the shape genuinely IS a valid
      // CreateDsrInput by this point; TS just can't see across that guard.
      const payload = buildPayload() as unknown as CreateDsrInput;
    ...
    <div className="mx-auto max-w-160">
      ...
      <form
        onSubmit={(event) => {
          ...
          // Revised 2026-09-24 (user-requested): Work note and Site
          // Contract are now required on every started Subcontractor row —
          // checked before even opening the confirm dialog, since silently
          // excluding a filled-in row would lose data the user typed with
          // no indication why.
          ...
          {subcontractorEntries.map((row, index) => (
            ...
            >
              ...
              {/* Revised 2026-09-24 (user-requested): both required now —
                  every DSR-tagged Subcontractor must carry a note and link
                  to a real Site Contract (any status). Only Quantity stays
                  optional (Work Entries need an Active contract). */}
              <div className="sm:col-span-5">
                <ComboboxField
                  label="Site Contract"
                  // Deliberately NOT using the `required` prop here: it
                  // drives native HTML5 validation off Combobox.Input's
                  // resolved *display* value (options.find(...)), not the
                  // underlying row.siteContractId state — if the matching
                  // contract isn't in the currently-loaded options list yet
                  // (async fetch timing), the input reads as empty and the
                  // browser silently blocks the whole form's submit event
                  // before the onSubmit guard's own (state-accurate) check
                  // ever runs. That guard is the real enforcement; this
                  // hint is just the visual cue.
                  hint="Required"
          ...
          {labourEntries.map((row, index) => (
            ...
            >
              ...
              {/* Own row, generously wide (1/3 each) — a narrower shared row
                  ...
                  TextField (unlike ComboboxField) applies `className` to
                  its inner <input>, not its outer wrapper — col-span must
                  go on a wrapping div, same pattern as "Quantity completed"
                  in the Subcontractor section above. */}
              <div className="sm:col-span-4">
                <TextField
                  label="Men"
                  type="number"
        ...
        {draftId ? (
          <div className="flex flex-col gap-2">
            <Button
              type="button"
              onClick={() => {
                // Revised 2026-09-24 (user-requested): same Work note /
                // Site Contract required-fields guard as the one-shot
                // Submit path above — Finalize bypasses the form's onSubmit
                // entirely (it's a plain button, not a submit input), so
                // this needs its own copy of the check.
                const incompleteSubcontractorRow = subcontractorEntries.find(
                  (s) => s.subcontractorId && (!s.workNote.trim() || !s.siteContractId),
                );
                if (incompleteSubcontractorRow) {
      ...
      >
        ...
        />
        <ConfirmDialogRow label="Labour" value={String(labourEntries.filter(isLabourRowComplete).length)} />
