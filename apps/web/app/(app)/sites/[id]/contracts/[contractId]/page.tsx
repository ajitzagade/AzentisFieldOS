...
// a failure here degrades only this ledger's table, never the whole page.
async function getWorkEntries(contractId: string): Promise<WorkEntryRow[] | null> {
  try {
    const res = await authedFetch(`/subcontractor-work-entries?siteContractId=${contractId}`, { cache: "no-store" });
    if (!res.ok) return null;
    const rows = (await res.json()) as WorkEntryRow[];
...
async function getPayments(contractId: string): Promise<SubcontractorPaymentRow[] | null> {
  try {
    const res = await authedFetch(`/subcontractor-payments?siteContractId=${contractId}`, { cache: "no-store" });
    if (!res.ok) return null;
    const rows = (await res.json()) as SubcontractorPaymentRow[];
    return Array.isArray(rows) ? rows : null;
  } catch {
    return null;
  }
...
export default async function SiteContractDetailPage({
  ...
  return (
    <>
      ...
      <div className="mb-8 rounded-lg border border-border-hairline bg-surface-1 p-6 shadow-2">
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-4 sm:gap-x-8">
          ...
          <div>
            ...
            <div className={cn("text-body-sm", contract.rate === null && contract.fixedAmount === null ? "italic text-ink-500" : "text-ink-900")}>
              {rateOrAmountLabel(contract)}
            </div>
          </div>
          <div>
      ...
      <div className={cn("mb-8 grid grid-cols-1 gap-4", showsQuantity ? "sm:grid-cols-4" : "sm:grid-cols-3")}>
        {showsQuantity ? (
          <StatTile
            icon={<ArrowsIcon />}
            tint="teal"
        ...
        <StatTile
          icon={<WalletIcon />}
          tint="gold"
          value={contract.amountPayable === null ? "Pending terms" : `₹${contract.amountPayable.toLocaleString("en-IN")}`}
          label="Amount payable"
        />
        <StatTile
          icon={<WalletIcon />}
          tint="gold"
          value={`₹${Number(contract.amountPaid).toLocaleString("en-IN")}`}
          label="Amount paid"
        />
        <StatTile
          icon={<WalletIcon />}
      ...
      <div className="mb-8">
        <DataTable
          ...
          state={
            workEntries === null
              ? { status: "empty", icon: <ClipboardIcon />, message: "Couldn't load Work Entries right now." }
      ...
      <DataTable
        ...
        state={
          payments === null
            ? { status: "empty", icon: <WalletIcon />, message: "Couldn't load Payments right now." }
            : payments.length === 0
