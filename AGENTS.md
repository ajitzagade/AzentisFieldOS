...
- **TODO:** `apps/api`'s actual dev-server boot path (`nest start` / `pnpm --filter @azentisfieldos/api start`, and by extension `pnpm dev`) currently crashes with a Node ESM module-resolution error loading `@azentisfieldos/shared` (whose `main` field points at raw `.ts` source — see the Turbopack-compatibility note above). Never exercised until Epic 3 (Epic 1/2 only ran `tsc`/`vitest`/`next build`, never an actually-running `apps/api` server). `tsx apps/api/src/main.ts` doesn't work either — NestJS needs `emitDecoratorMetadata`, which esbuild-based transforms don't emit (same reason `apps/api/vitest.config.mts` needs `unplugin-swc`). Needs a real fix (likely a dev-mode loader/register hook, or building `packages/shared`/`packages/ui` to real `.js` for `apps/api`'s consumption specifically) before `apps/api` can run outside of Vitest.
- **TODO:** DSR/photo/expense/RMC-entry endpoints currently use a placeholder "system" `User` row for `submittedByUserId`/`uploadedByUserId` — no request-level auth is wired into `apps/api` yet (Epic 1 scoped Clerk to `apps/web` only). Replace with the real authenticated user id once `apps/api` validates a Clerk session per-request (AD-10).
- **TODO:** `apps/api/src/storage/r2-client.ts` and the `POST /photos/presign`/`POST /photos` endpoints (story 3.3) are implemented and unit-tested against a mocked `@aws-sdk/s3-request-presigner`, but never exercised against a real Cloudflare R2 bucket — no `R2_*` credentials exist in any environment this codebase has run in yet. Verify the actual presigned-URL upload/download round-trip once a real bucket exists (`infra/provisioning`'s R2 TODO, above). Also unaddressed: if the DSR submission itself falls into story 3.2's offline queue, staged photos in the mobile form do not auto-upload once that queued submission later syncs — a fully offline-durable photo queue is real follow-up work.
