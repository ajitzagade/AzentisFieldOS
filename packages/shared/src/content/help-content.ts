...
export const HELP_CONTENT = {
  ...
  modules: [
    ...
    },
    {
      ...
      afterSaving: "The material is ready to appear in every Purchase, Movement, and Material Used form from now on — always with the same unit, so nobody has to guess what \"50\" means.",
    ...
    },
    {
      ...
      usedBy: ["OWNER_ADMIN", "SITE_SUPERVISOR"],
      howToUse: [
        "Open Movements → Record Material Purchase.",
    ...
    },
    {
      ...
      usedBy: ["OWNER_ADMIN", "SITE_SUPERVISOR"],
      howToUse: [
        "Open Movements → Godown to Site (or Transfer Site to Site for site-to-site).",
        "Pick the material and how much is being sent.",
        "Pick where it's going.",
        "Save — stock updates at both ends immediately.",
      ],
      afterSaving: "Stock decreases at the sending location and increases at the receiving location right away. If the amount that actually arrived turns out to be different, that's fixed afterward through Correct, the same as any other entry — never a separate required step.",
      example: "20 bags moved from Site A to Site B → Site A drops to 30 bags, Site B rises to 20 bags immediately.",
      href: "/movements",
    },
    {
      id: "consumption",
      ...
      howToUse: [
        "Open Movements → Record Material Used (or record it inside today's Daily Report).",
    ...
    },
    {
      ...
      usedBy: ["OWNER_ADMIN", "SITE_SUPERVISOR"],
      howToUse: [
        "Open Waste Material → Record Waste Material.",
  ...
  dayInTheLife: [
    ...
    { time: "Work happens", title: "50 bags cement used", detail: "Recorded as Material Used — stock drops to 50." },
  ...
  guides: [
    {
      ...
      title: "How to record material used",
      steps: [
        { title: "Open Movements", detail: "From the sidebar, open Movements, then Record Material Used — or add it directly inside today's report." },
        { title: "Site", detail: "Pick the site where the material was used. The form remembers the site you picked last time." },
        { title: "Material / Size", detail: "Type the material's name and pick it from the list. It shows how much stock the site has right now. If the material isn't in the list, tap + Add Material to add it on the spot." },
        { title: "Quantity", detail: "Type how much was used — for example, 50 bags. The unit is shown right on the box. If a warning says the quantity is more than the site's stock, re-check the number — the system won't accept more than what's on hand." },
        { title: "Used Date", detail: "Today's date is already filled in. Change it only if you're recording an earlier day's usage." },
        { title: "Activity Reference", detail: "You can leave this empty. If you like, note what the material was used for — for example, \"First-floor slab\"." },
        { title: "Notes", detail: "You can leave this empty. Add anything else worth remembering about this entry." },
        { title: "Record Material Used", detail: "Tap Record Material Used — that's it, the entry is saved." },
      ],
    ...
    },
    {
      ...
      steps: [
        ...
        { title: "Name", detail: "Type the site's name — for example, \"Riverside Tower\"." },
        { title: "Location", detail: "Type where the site is — for example, \"12 MG Road, Pune\"." },
        { title: "Status", detail: "Already set to Active — leave it as it is for a new site. You can change it to On Hold or Completed later." },
        { title: "Contract reference", detail: "You can leave this empty. If the project has a contract or work-order number, type it here." },
        { title: "Description", detail: "You can leave this empty. A line or two about the project, if you want it on record." },
        { title: "Create Site", detail: "Tap Create Site — that's it, the site is saved." },
      ],
    ...
    },
    {
      ...
      steps: [
        ...
        { title: "Start a new purchase", detail: "Tap Record Material Purchase." },
        { title: "Vendor", detail: "Type the vendor's name and pick them from the list — the supplier the material was bought from. If the vendor isn't in the list, tap + Add Vendor to add them on the spot." },
        { title: "Material / Size", detail: "Type the material's name and pick it from the list. It shows how much stock the Godown or site already has. If the material isn't in the list, tap + Add Material to add it on the spot." },
        ...
        { title: "Open More details", detail: "Tap \"More details — challan no., photo, vehicle, receiver, notes\" to open the remaining boxes. Every one of them is optional." },
        { title: "Invoice / Challan No.", detail: "You can leave this empty. The number printed on the bill or challan, if you have it." },
        { title: "Challan Photo", detail: "You can leave this empty. Take a photo of the challan so the paper is never lost." },
        { title: "Delivery Location", detail: "You can leave this empty. Where exactly the material was dropped off." },
        { title: "Vehicle Details", detail: "You can leave this empty. The delivery vehicle's number — for example, MH12AB1234." },
        { title: "Receiver Name", detail: "You can leave this empty. Who accepted the delivery — pick a team member or type any name." },
        { title: "Notes", detail: "You can leave this empty. Anything else worth remembering about this purchase." },
        { title: "Record Material Purchase", detail: "Tap Record Material Purchase — that's it, the purchase is saved." },
      ],
    ...
    },
    {
      ...
      steps: [
        ...
        { title: "Payment Method", detail: "You can leave this empty. How the money was given — for example, Cash or Bank Transfer." },
        { title: "Record Advance", detail: "Tap Record Advance, check the amount in the confirmation box, then tap Confirm & Submit." },
      ],
    ...
    },
    {
      ...
      steps: [
        { title: "Open today's report", detail: "Tap Start Daily Report on your Home screen, or Report in the bar at the bottom of your phone." },
        { title: "Site", detail: "Pick your site — the form remembers the one you picked last time. You can't submit without it." },
        { title: "Date", detail: "Today's date is already filled in. Change it only if you're filing an earlier day's report. Set the site and date first — changing either one clears any photos you've already added." },
        { title: "Work completed", detail: "Write a line or two about today's work — for example, \"First-floor slab casting done\". You can leave it empty, but a short note helps the office." },
        { title: "Issues / blockers", detail: "You can leave this empty. Note anything that stopped or slowed the work today." },
        { title: "Crew present today", detail: "The list starts with the crew from the last report — untick anyone who was absent. For a site's first-ever report the list starts empty, so add everyone. To add someone, type their name under Add crew member and pick them; if they're not in the list, tap + Add Team Member." },
        { title: "Materials consumed", detail: "Tap Add material for each material used today. Each row has two boxes: Material (type the name and pick it) and Quantity — for example, 50 bags. Fill every box in a row — a half-filled row is not saved (the same goes for RMC and Expense rows) — or remove the row." },
        { title: "RMC (ready-mix concrete) used", detail: "Tap Add RMC delivery for each concrete delivery. Each row has: Vendor, Quantity (m³), Grade (for example, M25), and Rate per m³." },
        { title: "Expenses", detail: "Tap Add expense for money spent today. Each row has: Category, Amount — for example, ₹2,000 for diesel — and a Description you can leave empty." },
        { title: "Equipment used today", detail: "Type a machine name or vehicle number under Add machinery or vehicle and pick it from the list. If it isn't in the list, it must first be added under Machinery & Vehicles — you can't add it from here." },
        { title: "Site Photos", detail: "Tap the camera box to take photos or choose them from the gallery." },
        { title: "Submit Daily Report", detail: "Tap Submit Daily Report. Even without network signal, the written report is saved on the phone and syncs by itself once you're back online — but photos don't wait that way: they upload only while the app is open, so if you were offline, open the app again when you have signal and check the photos went through." },
      ],
...
export const SEARCH_ACTIONS: SearchAction[] = [
  ...
  },
  {
    ...
    description: "Save a site measurement with photos",
    keywords: ["measurement", "add measurement", "site measurement"],
    href: null,
  },
  {
    id: "record-advance",
  ...
  {
    id: "record-movement",
    title: "Godown to Site",
    description: "Send material from the Godown to a Site",
    keywords: ["record movement", "record material movement", "material sent", "godown to site"],
    href: "/movements/godown-to-site/new",
  },
  {
    id: "record-transfer",
    title: "Transfer Site to Site",
    description: "Send material from one Site to another",
    keywords: ["record transfer", "record material transfer", "material sent", "site to site", "transfer"],
    href: "/movements/site-to-site/new",
  },
  {
    id: "record-consumption",
    title: "Record Material Used",
    description: "Log material used today",
    keywords: ["record consumption", "record material used", "material used"],
    href: "/movements/consumption/new",
  },
  {
    id: "record-wastage",
    title: "Record Material Wastage / Return",
    description: "Log material wasted or returned",
    keywords: ["record wastage", "record material wastage", "wastage", "return"],
